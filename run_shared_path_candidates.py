...
def run_candidate(requests, metadata, candidate):
    ...
    # The production adapter now supports its own JIT hook. Candidates own
    # their release rule here, so suppress that hook to avoid applying JIT twice
    # (and to keep non-JIT assignment ablations genuinely non-JIT).
    with patch.object(shared_path_strategy1, "choose_npu", choose), \
         patch.object(shared_path_strategy1, "prefetch_release_ms",
                      lambda now_ms, *args, **kwargs: now_ms), \
