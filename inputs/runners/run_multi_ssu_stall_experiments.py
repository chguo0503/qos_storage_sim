...
from simulator.config import qos_configs_from_path_cirs, routing_strategy_specs, static_qos_config
...
def run_case(requests, metadata, *, strategy="baseline", assignment="none", min_interval_ms=0.0):
    ...
    if strategy not in ("baseline", "layer_once"):
        from simulator.adapters.multi_ssu_qos_controller import MultiSSUController, multi_ssu_control_events
    ...
    assign = nullcontext([])
    if assignment != "none":
        from simulator.adapters.multi_ssu_npu_assignment import assign_requests_on_arrival
    ...
    assert all(hashlib.sha256((ROOT / name).read_bytes()).hexdigest() == hashes[name] for name in source_names), "source changed during experiment"
...
def main():
    ...
    parser.add_argument("--strategy", choices=("baseline", "layer_once", "demand", "deadline",
                                               "deadline_reserve", "least_slack", "stall_interchange"), default="baseline")
