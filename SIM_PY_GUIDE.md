...
### 10.3 active 是否表示多条 flow 同时完成

不是。<code>active</code> 只表示这条 flow 已经进入服务窗口，当前可以分到带宽并随仿真时间向前传输；<code>pending</code> 表示仍在 FCFS 等待区，当前带宽为 0。调度器也不是先“选中一条 queue 并一次做完”，而是让所有活跃 queue 同时分享 SSU 总带宽，再把每条 queue 的份额分给其中的 active flow。

例如某 queue 得到 12 GB/s，其中 3 条等权 active flow 分别还剩 0.004、0.008、0.012 GB。若带宽保持不变，它们各得约 4 GB/s，将分别在 1、2、3 ms 后完成。它们是同时传输，不是同时完成。只有剩余数据量、开始时间和带宽份额相同，并且中途没有重新分配时，才可能在同一时刻完成。最早的 flow 完成时，事件循环会把所有 active flow 都推进到该时刻，然后从 pending 队头补位并重新分配带宽。

<code>max_depth=4</code> 也不会让一条 queue 获得 4 倍带宽。它主要减少队头阻塞：深度为 1 时，队头的大 flow 可以让后面另一个 NPU 的关键层数据完全不前进；深度为 4 时，多个 NPU 的 flow 分享同一份 queue 带宽，较小或更早需要的 flow 可能先完成，从而更早解除 NPU 的 layerwise I/O 等待。传完该 queue 全部字节的总时间可能相同，但各请求的完成顺序和 NPU 利用率会不同。

因此深度 4 并非在任何负载下都必然更好：并发分享也可能让原本队头的 flow 变慢。当前混合负载结果更好，表示减少其他 NPU 队头等待的收益大于队头 flow 被分流的代价。
...
### 24.4 当前：256 队列、固定与 NPU 自适应发批结果

q256 使用 8 个等权 group，每组 32 条共享队列；所有 group 权重为 1，单队列 CIR 为 0.15625 GB/s，active depth 同样为 4。其余输入与 q24 完全相同：

| SSU | fair | baseline_qwrr | Batch fixed | TA fixed | Batch adaptive | TA adaptive |
| ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| 16 | 47.0% | 48.7% | 48.7% | 48.8% | 48.4% | 48.2% |
| 28 | 59.9% | 62.0% | 51.3% | 51.3% | 62.0% | 61.7% |
| 40 | 68.7% | 70.7% | 51.3% | 51.3% | 71.0% | 70.8% |
| 56 | 77.4% | 79.5% | 51.3% | 51.3% | 80.4% | 80.3% |
| 84 | 88.4% | 88.8% | 51.4% | 51.3% | 90.3% | 91.2% |
| 112 | 93.2% | 92.5% | 51.4% | 51.3% | 94.4% | 94.6% |

完整 q256 运行的退出码为 0，墙钟耗时 3704.71 秒；缓存写入后再次运行只需 0.95 秒。缓存保存在 <code>results/io_scheduling_sweep_q256_qd4_b200us_pir_uncapped.pkl</code>，图片使用 <code>results/io_sched_16L_*_q256_qd4_b200us.png</code> 文件名。

相对 q24，q256 baseline QWRR 在 16 SSU 时提高 3.3 个百分点，随后差距逐步收敛，到 112 SSU 只高 0.1。固定 200 µs 的 Batch/TA 在两种队列配置下几乎完全相同，因为此时瓶颈是 NPU 发批节奏，不是 QoS queue 数量。自适应 Batch 的 q256 相对 q24 在 16、28、40、56、84、112 SSU 分别提高约 2.1、0.9、0.8、1.7、0.9、0.0 个百分点；自适应 TA 的差值分别约为 1.8、0.7、0.7、1.5、0.5、0.0。

这些差异仍不能只归因于队列数量：q24 使用三档 CIR 和 4:2:1 借用权重，q256 使用 8 个等权 group。当前合并图 <code>results/io_sched_16L_q24_vs_q256_qd4_b200us.png</code> 同时画出了 baseline、固定 Batch/TA 和自适应 Batch/TA 的 q24/q256 曲线。
