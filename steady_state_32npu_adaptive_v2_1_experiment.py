"""Isolated 32-NPU real-data validation for Adaptive Admission V2.1.

The frozen matrix contains one 25-ms event-gated adaptive policy at SSU6/10/18.
It writes a private checkpoint with complete simulator summaries and provenance;
no existing experiment, controller, or source-fingerprint list is modified.
"""
...
from steady_state_32npu_experiment import N_LAYERS, ROOT, STEADY_CONFIG
from steady_state_workload import REQUESTS_PER_NPU, prepare_steady_state_workload


OUTPUT_DIR = ROOT / "results" / "steady_state_32npu_adaptive_v2_1"
...
def _source_fingerprint():
    digest = hashlib.sha256(b"steady-state-32npu-adaptive-v2-1\0")
...
def experiment_spec():
    return {
        ...
        "experiment": "isolated_32npu_adaptive_admission_v2_1",
        "num_npu": NUM_NPU,
        "n_layers": N_LAYERS,
        "batch_size": 1,
...
def _config_fingerprint(spec=None):
    return _canonical_hash(
        spec or experiment_spec(), b"32npu-adaptive-v2-1-config-v1\0"
    )
...
def _case_fingerprint(case, source_fingerprint, config_fingerprint):
    return _canonical_hash(
        ...
        b"32npu-adaptive-v2-1-case-v1\0",
    )
...
def _validate_summary(case, summary):
    ...
    mode_counts = summary.get("adaptive_residual_mode_evaluations", {})
    if sum(mode_counts.values()) != summary["control_evaluations"]:
...
def _category_slo(rows):
    return {
        category: statistics.fmean(
            bool(row["slo_met"]) for row in rows if row["category"] == category
        )
...
def render_markdown(payload):
    lines = [
        ...
        "",
        "Adaptive V2.1 uses the current active-manifest selected fraction. "
        "Fractions strictly below 0.75 select explicit V2 spill; all others "
        "retain V1 coflow residual. The signal is causal, while 0.75 is a "
        "calibrated operating-point heuristic rather than a universal optimum.",
        "",
