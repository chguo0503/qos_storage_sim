"""Analyze modified baseline/Scheme-B cold and warm six-request metrics."""
...
def _validate(payload):
    ...
    for n_layers in sorted({row["n_layers"] for row in rows}):
        for num_ssu in sorted(
            ...
            if {row["strategy"] for row in group} != {
                case.name for case in CASES
            }:
                raise ValueError(
                    f"incomplete strategy pair at layers={n_layers}, SSU={num_ssu}"
                )
            for field in (
                "assignment_hash",
                "workload_hash",
...
def analyze(payload):
    ...
    for n_layers in layers:
        deltas[str(n_layers)] = {}
        for num_ssu in ssus:
            baseline = metrics["modified_baseline"][str(n_layers)][str(num_ssu)]
            scheme_b = metrics["modified_scheme_b"][str(n_layers)][str(num_ssu)]
            deltas[str(n_layers)][str(num_ssu)] = {
                cohort: {
                    "npu_utilization_delta_pp": 100.0
                    * (
                        scheme_b[cohort]["mean_npu_utilization"]
                        - baseline[cohort]["mean_npu_utilization"]
                    ),
                    "ttft_slo_delta_pp": 100.0
                    * (
                        scheme_b[cohort]["ttft_slo"][_alpha_key(PLOT_ALPHA)][
                            "attainment"
                        ]
                        - baseline[cohort]["ttft_slo"][_alpha_key(PLOT_ALPHA)][
                            "attainment"
                        ]
                    ),
                }
    ...
    return {
        ...
        "metrics": metrics,
        "modified_scheme_b_minus_modified_baseline": deltas,
        "validation": {
            "paired_inputs": True,
...
def write_report(path, analysis):
    ...
    for cohort in ("cold", "warm"):
        lines.extend(
            [
                "",
                f"## Scheme B minus baseline — {cohort}",
                "",
        ...
        for n_layers in analysis["layer_list"]:
            values = [
                analysis["modified_scheme_b_minus_modified_baseline"][
                    str(n_layers)
