...
def prepare(spec_path):
    ...
    import mixed_design_v8 as design
    items=[]
    for spec in read_json(spec_path):
        for seed in spec.get('seeds',[7]):
            source=FOLLOWUP/'inputs'/'raw176_three_l20_fixed_seed7.json.gz'
            ...
            label=f"raw176_extendedhot_{spec['mode']}_seed{seed}"
            metadata=dict(meta)
            ...
            metadata.update(label=label,case_id=f'{label}_{fp[:12]}',input_fingerprint=fp,
                ...
                ordering_scope='Global original raw176 seed7 population is rebound once per seed; random and ordered have identical per-NPU populations, placement, C/V and zero arrivals. No artificial delay.')
            path=HERE/'inputs'/f'{label}.json.gz'
            save_manifest(path,new,metadata)
            items.append(dict(label=label,spec_name='raw176_extendedhot',seed=seed,mode=spec['mode'],
