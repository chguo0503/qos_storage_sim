"""Backup single-handoff queue permutations; no simulation or timing controls.

This module is independent of frozen mixed_design.py.  All wall-time values
...
VARIANTS = ("handoff_10l_344", "handoff_guard4_344", "handoff_guard4_434")
...
EARLY_HANDOFF_TARGET_MS = 3800.0
...
def _rng(seed, npu, profile):
    encoded = f"single_handoff_v2:{int(seed)}:{int(npu)}:{profile}".encode()
...
def order_lane(lane, metadata, npu, seed, variant):
    ...
    the remaining 20 cards a common Short prefix and a 9/10-Long middle segment.
    ...
    if any(min(c[:3]) < 5 or c[3] < 10 for c in source_counts.values()):
    ...
    if int(npu) in early_cards:
        ...
        intended_main_long_count = 9 if int(npu) in guard_cards else 10
    ...
    desc = dict(design_version="single_handoff_v2", variant=variant, npu=int(npu), seed=int(seed),
        ...
        early_selection_rule="12 cards nearest 3*C_Long+sum(all Short counts*fixed-20L wall proxy)=3800ms, ties by NPU",
        ...
        late_10L_pure_compute_ms=10 * computes[3], late_9L_pure_compute_ms=9 * computes[3],
        late_prefix_latest_end_for_10L_then_100ms_compute=3900.0 - 10 * computes[3],
        ...
        limitations=("No absolute-time guarantees or delays. Initial Short prefixes run against 12 Long cards, "
                     "so a fixed-20L processing proxy is not their predicted completion time. The 28-Long "
                     "guard is conditional on all four 9L segments ending before early cards rejoin, and "
