...
import mixed_design_v7 as design
import mixed_design_v6 as previous
...
def main():
    ...
    for seed in (7, 19, 43, 67, 101):
        ...
        old_random, old_desc = previous.build_queues(source, metadata, seed, "random")
        assert all(all(a is b for a, b in zip(random[n], old_random[n])) and len(random[n]) == len(old_random[n]) for n in range(32))
        ...
        for npu in range(32):
            ...
            expected = [3, 3, 3, 13] if npu < 17 else [8, 8, 8, 8] if npu < 20 else [16 if npu < 29 else 15] * 3 + [12 if npu < 23 else 11]
            ...
            lanes.append(dict(npu=npu, quotas=actual, pure_compute_ms=pure_compute,
                paired_original_objects=True, v6_random_exact_original_object_order=True, deterministic_despite_source_enumeration=True,
    ...
    output = dict(scope="Pure generation checks; no simulation or claims about warm mix/actual capacity/U",
        ...
        v6_reference_sha256=hashlib.sha256(Path(previous.__file__).read_bytes()).hexdigest(),
        source_inputs=len(rows), modes_per_source=2, lanes_paired=32 * len(rows), all_checks_pass=True, rows=rows)
    (HERE / "mixed_design_v7_checks.json").write_text(json.dumps(output, indent=2) + "\n")
