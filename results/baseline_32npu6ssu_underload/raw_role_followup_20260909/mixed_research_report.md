...
|---|---|---:|---:|---:|---:|---:|---|
| 176K：初版交接 | random | 1 | 99.99 | 99.88 | 100.00 | 100.00 | 2/2 |
| 176K：初版交接 | ordered | 1 | 91.62 | 99.22 | 97.90 | 100.00 | 2/2 |
| 200K：较晚交接（Once超限） | random | 1 | 99.95 | 99.84 | 100.00 | 100.00 | 2/2 |
| 200K：较晚交接（Once超限） | ordered | 1 | 88.80 | 98.82 | 74.89 | 100.00 | 1/2 |
| 200K：三张卡提前切换 | random | 1 | 99.90 | 99.84 | 100.00 | 100.00 | 2/2 |
| 200K：三张卡提前切换 | ordered | 1 | 90.42 | 99.12 | 87.03 | 100.00 | 2/2 |
| 200K：三种短画像交错 | ordered | 1 | 91.30 | 99.06 | 92.10 | 100.00 | 2/2 |
| 176K：延长短请求阻塞段 | random | 5 | 99.98 ± 0.01 | 99.93 ± 0.03 | 100.00 ± 0.00 | 100.00 ± 0.00 | 10/10 |
| 176K：延长短请求阻塞段 | ordered | 5 | 89.53 ± 0.04 | 99.27 ± 0.05 | 96.68 ± 0.45 | 100.00 ± 0.00 | 10/10 |

同一行的两策略使用完全相同manifest；同一分配的random与ordered保留每张卡原请求集合、C/V、arrival和逐块SSU位置，只改队列顺序。不同分配版本改变了每卡配额，不能把它们之间的差异都归因于排序。三种短画像交错方案复用“提前切换”方案的随机对照，输入指纹完全相同，不计作额外独立运行。
...
|---|---|---:|---:|---:|---:|---:|
| raw176_three_l20 | 20长卡/12短卡 | 91.28 ± 0.14 | 100.00 ± 0.00 | 92.38 ± 1.93 | 100.00 ± 0.00 | 5 |
| raw176_three_l20 | 均匀分配后每卡随机混合 | 99.96 | 99.81 | 100.00 | 100.00 | 1 |
| raw200_three_l20 | 20长卡/12短卡 | 88.45 ± 0.12 | 100.00 ± 0.01 | 82.53 ± 2.08 | 100.00 ± 0.00 | 5 |
| raw200_three_l20 | 均匀分配后每卡随机混合 | 99.91 ± 0.05 | 99.85 ± 0.06 | 100.00 ± 0.00 | 100.00 ± 0.00 | 5 |

176K各版本保留同一全局1212请求人口：420条L、S1/S2/S3各264条。200K各版本保留同一全局1172人口：380条L、三种S各264条。176K和200K之间不是同人口比较。已有五seed的160K及构造输入完整表见[原始数据与固定分卡报告](research_report.md)。
...
$\mathrm{stall}=\max(0,\ t_{\mathrm{IO\ ready}}-t_{\mathrm{previous\ compute\ end}})$。
...
|---|---:|---|---|---:|---:|---:|
| 176K：初版交接/baseline | 1 | 21.22/10.78 | 12–29 | 65.78% | 202.28 | 39.535751 |
| 176K：初版交接/once | 1 | 26.56/5.44 | 12–32 | 23.56% | 171.17 | 39.952277 |
| 200K：较晚交接（Once超限）/baseline | 1 | 18.70/13.30 | 12–29 | 58.46% | 283.73 | 39.814817 |
| 200K：较晚交接（Once超限）/once | 1 | 25.74/6.26 | 12–32 | 23.56% | 240.61 | 40.243858 |
| 200K：三张卡提前切换/baseline | 1 | 18.56/13.44 | 12–29 | 15.16% | 286.40 | 39.819553 |
| 200K：三张卡提前切换/once | 1 | 24.77/7.23 | 12–29 | 16.80% | 229.10 | 39.829024 |
| 200K：三种短画像交错/baseline | 1 | 18.02/13.98 | 12–29 | 15.73% | 288.04 | 39.814817 |
| 200K：三种短画像交错/once | 1 | 23.99/8.01 | 12–29 | 16.04% | 237.51 | 39.829024 |
| 176K：延长短请求阻塞段/baseline | 5 | 18.35/13.65 | 0–20 | 64.61% | 166.98 | 38.313281 |
| 176K：延长短请求阻塞段/once | 5 | 24.51/7.49 | 12–32 | 41.35% | 145.82 | 39.946934 |

长短卡数按“当前已接纳哪一类请求”统计，包含该请求的I/O等待。固定对照始终20/12；混合只力求时间平均接近，不是任意时刻都相同。Once更快完成短请求，会改变长短驻留比例，不能要求同一输入在两个策略下还保持同一个角色时序。
