"""New binding plus paired queue orders; original raw200 objects are retained.
...
def _rng(seed, purpose):
    text = f"raw200_rebinding_v6:{int(seed)}:{purpose}".encode()
...
def _quota(npu):
    ...
    if npu < 20:
        return [8, 8, 8, 8]
    position = npu - 20
    short_count = 16 if position < 9 else 15
    return [short_count] * 3 + [12 if position < 3 else 11]
...
def build_queues(source_requests, source_metadata, seed, mode):
    ...
    lanes use shuffled 9*S2+8*S3, then shuffled11*S1+2*S2+2*S3, then all Long, then remaining Short.
    """
    ...
    if len(source) != 1172 or len({r.request_id for r in source}) != 1172:
        raise ValueError("expected exactly 1172 unique source requests")
    ...
    if [len(pools[p]) for p in range(4)] != [264, 264, 264, 380]:
        raise ValueError("source global population does not match v7 design")
    computes = []
    ...
    assert [sum(q[p] for q in quotas.values()) for p in range(4)] == [264, 264, 264, 380]
    ...
    for npu in range(32):
        ...
        if mode == "random":
            ...
            if npu < 20:
                ...
                hot = [0] * 11 + [1] * 2 + [2] * 2
                _rng(seed, f"ordered_hot_short_v7_{npu}").shuffle(hot)
                tail_counts = [quota[0] - 11, quota[1] - 11, quota[2] - 10]
                template = prefix + hot + [3] * quota[3] + _round_robin(tail_counts)
                segment_description = "independent shuffle of9*S2+8*S3; then shuffled11*S1+2*S2+2*S3; then all Long; then remaining Short"
    ...
    description = dict(design_version="raw200_same_binding_varied_hot_v7", mode=mode, seed=int(seed),
        requests=1172, profile_keys=[list(x) for x in PROFILE_KEYS], global_profile_counts=[264, 264, 264, 380],
        ...
        population_rule="Same1172 global original requests. Per-card population is changed once by binding, then identical for random and ordered.",
        ...
        ordered_short12_hot_counts=[11, 2, 2, 0],
        random_and_assignment_namespace="raw200_rebinding_v6 (intentionally identical to v6)",
        ordered_short12_total_prefix_pure_compute_ms=11 * computes[0] + 11 * computes[1] + 10 * computes[2],
        ...
        metadata_fields_to_replace={"assignment_mode": "same_binding_varied_hot_v7", "order": mode,
            ...
            "population_rule": "Same global1172 original raw200 requests; new per-card quotas paired exactly between random and ordered",
