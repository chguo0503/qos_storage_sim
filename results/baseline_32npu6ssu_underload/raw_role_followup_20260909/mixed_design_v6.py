...
def build_queues(source_requests, source_metadata, seed, mode):
    ...
    lanes use shuffled 9*S2+8*S3, then15*S1, then all Long, then remaining Short.
    """
    ...
    if [len(pools[p]) for p in range(4)] != [264, 264, 264, 380]:
        raise ValueError("source global population does not match v6 design")
    computes = []
    ...
    for npu in range(32):
        ...
        if mode == "random":
            ...
            if npu < 20:
                ...
                tail_counts = [quota[0] - 15, quota[1] - 9, quota[2] - 8]
                template = prefix + [0] * 15 + [3] * quota[3] + _round_robin(tail_counts)
                segment_description = "independent shuffle of9*S2+8*S3; then15*S1; then all Long; then remaining Short"
    ...
    description = dict(design_version="raw200_new_binding_v6", mode=mode, seed=int(seed),
        ...
        ordered_short12_following_S1_count=15,
        ordered_short12_total_prefix_pure_compute_ms=15 * computes[0] + 9 * computes[1] + 8 * computes[2],
        ...
        metadata_fields_to_replace={"assignment_mode": "new_binding_mixed_v6", "order": mode,
