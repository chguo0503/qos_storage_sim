...
def main():
    ...
    if ordered['status']=='complete':
        ...
        for case in cases:
            ...
            after=[r for r in rs if r['release_ms']>=max(guard_times)]
            case['selected_short_before_first_guard_exit']=max(before,key=lambda r:(r['stall_ms'],-r['release_ms'])) if before else None
            case['selected_short_after_all_guard_exit']=max(after,key=lambda r:(r['stall_ms'],-r['release_ms'])) if after else None
            case['short_internal_by_ordered_guard_epoch']={
                'before_first_guard_exit':layer_stats(before),
                'after_all_guard_exit':layer_stats(after),
    ...
    md+=['','具体短内部层：Fixed取完整暖窗最大等待；Ordered取guard退出前最大、全暖窗最大；两者相同则以guard退出后最大作为第二条，避免重复同一层。原始Random样例另存JSON。以下不是查明FIFO前驱的样本。','',
    ...
    if ordered['status']=='complete':
        before=ordered.get('selected_short_before_first_guard_exit');largest=ordered['selected_short_example']
        if before and largest and (before['request_id'],before['layer'])==(largest['request_id'],largest['layer']):
            examples.extend([('ordered／guard前且全暖窗最大',before),('ordered／guard退出后最大',ordered.get('selected_short_after_all_guard_exit'))])
        else:
            examples.extend([('ordered／guard退出前',before),('ordered／全暖窗最大',largest)])
