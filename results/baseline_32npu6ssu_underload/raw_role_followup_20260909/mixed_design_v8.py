...
def _rng(seed, purpose):
    text = f"raw176_rebinding_v8:{int(seed)}:{purpose}".encode()
...
def _quota(npu):
    ...
    if npu < 20:
        return [0, 6, 6, 13]
    position = npu - 20
    return [22, 15 if position == 0 else 14, 15 if position < 2 else 14,
            11 if position < 6 else 10]
...
def build_queues(source_requests, source_metadata, seed, mode):
    ...
    lanes use shuffled 9*S2+8*S3, then22*S1, then all Long, then remaining Short.
    """
    ...
    if [len(pools[p]) for p in range(4)] != [264, 264, 264, 420]:
        raise ValueError("source global population does not match v8 design")
    computes = []
    ...
    for npu in range(32):
        ...
        if mode == "random":
            ...
            if npu < 20:
                ...
                tail_counts = [quota[0] - 22, quota[1] - 9, quota[2] - 8]
                template = prefix + [0] * 22 + [3] * quota[3] + _round_robin(tail_counts)
                segment_description = "independent shuffle of9*S2+8*S3; then22*S1; then all Long; then remaining Short"
    ...
    description = dict(design_version="raw176_new_binding_v8", mode=mode, seed=int(seed),
        ...
        ordered_short12_following_S1_count=22,
        ordered_short12_total_prefix_pure_compute_ms=22 * computes[0] + 9 * computes[1] + 8 * computes[2],
        ...
        metadata_fields_to_replace={"assignment_mode": "new_binding_mixed_v8", "order": mode,
