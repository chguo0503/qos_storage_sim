...
def main():
    ...
    for seed in (7, 19, 43, 67, 101):
        path = HERE / "inputs" / f"raw176_three_l20_fixed_seed{seed}.json.gz"
        if not path.exists():
            continue
        ...
        for npu in range(32):
            ...
            expected = ([0, 5, 4, 15] if npu < 9 else [0, 4, 5, 15] if npu < 17 else [0, 6, 6, 13] if npu < 20 else [22, 15 if npu == 20 else 14, 15 if npu < 22 else 14, 11 if npu < 26 else 10])
    ...
    (HERE / "mixed_design_v8_checks.json").write_text(json.dumps(output, indent=2) + "\n")
