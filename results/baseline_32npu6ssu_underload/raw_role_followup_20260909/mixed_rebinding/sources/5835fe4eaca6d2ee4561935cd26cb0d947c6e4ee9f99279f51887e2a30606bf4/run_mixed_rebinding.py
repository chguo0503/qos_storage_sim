...
def prepare(spec_path):
    ...
    import mixed_design_v4 as design
    items=[]
    for spec in read_json(spec_path):
        for seed in spec.get('seeds',[7]):
            ...
            label=f"raw176_rebinding_{spec['mode']}_seed{seed}"
            metadata=dict(meta)
            ...
            items.append(dict(label=label,spec_name='raw176_rebinding',seed=seed,mode=spec['mode'],
