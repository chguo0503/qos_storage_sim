...
def _rng(seed, purpose):
    text = f"raw200_rebinding_v5:{int(seed)}:{purpose}".encode()
...
def build_queues(source_requests, source_metadata, seed, mode):
    ...
    if [len(pools[p]) for p in range(4)] != [264, 264, 264, 380]:
        raise ValueError("source global population does not match v5 design")
    computes = []
    ...
    description = dict(design_version="raw200_new_binding_v5", mode=mode, seed=int(seed),
        ...
        metadata_fields_to_replace={"assignment_mode": "new_binding_mixed_v5", "order": mode,
