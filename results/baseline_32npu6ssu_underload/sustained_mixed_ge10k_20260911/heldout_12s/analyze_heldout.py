...
def audit_result(item, q, m):
    ...
    if cmd['status'] != 'complete':
        observation=HERE/'interruption_observation.json'
        if observation.exists():
            observed=read(observation)
            match=next((x for x in observed['runs'] if x['pid']==cmd.get('pid')),None)
            if match and not match['process_exists'] and observed['wrapper_exit_code']==143:
                return dict(status='interrupted_unknown',pid=cmd.get('pid'),raw_command_status=cmd['status'],
                            strategy_failure=False,complete_result_available=False,observation=str(observation))
        return dict(status=cmd['status'],pid=cmd.get('pid'))
...
def main():
    ...
    if errors or (args.require_complete and output['complete']!=2):raise SystemExit(1)
