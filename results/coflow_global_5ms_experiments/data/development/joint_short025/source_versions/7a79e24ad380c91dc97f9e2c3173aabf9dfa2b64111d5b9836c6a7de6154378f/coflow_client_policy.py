...
def choose_npu_pipeline(backlogs, incoming_layer_io_by_ssu, compute_ms,
                        original_npu_id, *, n_layers=8,
                        disk_bw_gib_s=40.0, link_bw_gib_s=50.0):
    """Return (NPU ID, scores_ms) using known compute AND known I/O backlog.

    Score=max(remaining compute, receive service, equal-share disk service)
    after adding the new request. Active sharing counts already-arrived work,
    not future arrivals. This fluid overlap/equal-share estimate is not an
    actual finish-time prediction under category CIR or layer dependencies.
    Backlog rows expose remaining_compute_ms, unfinished_io_by_ssu and
    unfinished_request_count; no instantaneous SSD queue access is required.
    """
    incoming = tuple(n_layers * k for k in incoming_layer_io_by_ssu)
    active = [sum(r.unfinished_io_by_ssu[s] > 0 for r in backlogs)
              for s in range(len(incoming))]
    scores = []
    for row in backlogs:
        work = tuple(q + k for q, k in zip(row.unfinished_io_by_ssu, incoming))
        disk_ms = max((1000 * IO_GIB * k * (active[s] +
            (row.unfinished_io_by_ssu[s] == 0 and incoming[s] > 0)) / disk_bw_gib_s
                       for s, k in enumerate(work)), default=0.0)
        scores.append(max(row.remaining_compute_ms + n_layers * compute_ms,
                          1000 * IO_GIB * sum(work) / link_bw_gib_s, disk_ms))
    chosen = min(range(len(backlogs)), key=lambda n: (
        scores[n], backlogs[n].unfinished_request_count,
        backlogs[n].remaining_compute_ms, n != original_npu_id, n))
    return chosen, tuple(scores)
