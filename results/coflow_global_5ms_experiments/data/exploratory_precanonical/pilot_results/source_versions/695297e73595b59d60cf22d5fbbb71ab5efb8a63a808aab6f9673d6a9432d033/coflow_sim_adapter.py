...
class GlobalClient:
    def __init__(self, shared, policy, queue_window_ms, assignment):
        self.shared = shared
        self.policy = policy
...
def coflow_adapter(policy="strategy1", collector_interval_ms=5.0,
    ...
    with shared_path_adapter("new_once", collector_interval_ms, cir_min_interval_ms) as shared:
        client = GlobalClient(shared, policy, queue_window_ms, assignment)
        ...
        def schedule(context):
            if not context.submission_states or client.blocked:
                return
            earliest = min(max(state.ready_time_ms,
                               context.client_next_issue_ms[state.npu_id])
                           for state in context.submission_states.values())
            when = max(context.current_time_ms, client.next_issue_ms, earliest)
