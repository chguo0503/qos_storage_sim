...
class SharedPathAdapter:
    def __init__(self, strategy, collector_interval_ms=5.0,
        ...
        self.route_examples = []
        self.prefetch_stats = {"enabled": strategy in ("strategy1", "strategy2"),
            "guard_ms": 10.0, "activation_count": 0, "delay_count": 0,
            "total_delay_ms": 0.0, "max_delay_ms": 0.0,
            "decision_wall_us": 0.0, "release_examples": [],
    ...
    def statistics(self):
        return {
            **self.collector.statistics(),
            ...
            "jit_prefetch": self.prefetch_stats,
        }
...
def shared_path_adapter(strategy="once", collector_interval_ms=5.0,
    ...
    from shared_path_strategy1 import choose_npu, prefetch_release_ms
    ...
    original_start_io = native._start_layer_io
    ...
    def start_io(context, request, layer, now_ms, deadline_ms, demand_window_ms, **kwargs):
        # One common release time for all components of a not-yet-issued
        # coflow. Already queued I/O and ongoing compute are never postponed.
        if (strategy not in ("strategy1", "strategy2") or
                not 0 <= layer < context.n_layers or request.io_started[layer]):
            return original_start_io(context, request, layer, now_ms, deadline_ms,
                                     demand_window_ms, **kwargs)
        started = perf_counter()
        rows = adapter.layer_counts(request)
        counts = rows[0 if len(rows) == 1 else layer]
        pressure = tuple(adapter.collector.get(s, now_ms).counts
                         for s in range(context.num_ssu))
        release = prefetch_release_ms(now_ms, deadline_ms, counts, pressure,
            guard_ms=10.0, disk_bw_gib_s=context.disk_bw_gbps,
            link_bw_gib_s=context.npu_bw_gbps)
        stats = adapter.prefetch_stats
        stats["decision_wall_us"] += (perf_counter() - started) * 1e6
        before = context.next_submission_id
        result = original_start_io(context, request, layer, now_ms, deadline_ms,
                                   demand_window_ms, **kwargs)
        # Native client events honor ready_time_ms and reschedule themselves.
        # Preserve activation time for honest end-to-end wait accounting.
        for state_id in range(before, context.next_submission_id):
            state = context.submission_states[state_id]
            state.ready_time_ms = max(state.ready_time_ms, release)
        delay = max(0.0, release - now_ms)
        stats["activation_count"] += 1
        if delay > 1e-12:
            stats["delay_count"] += 1
            stats["total_delay_ms"] += delay
            stats["max_delay_ms"] = max(stats["max_delay_ms"], delay)
            if len(stats["release_examples"]) < 128:
                stats["release_examples"].append({"request_id": request.manifest.request_id,
                    "npu_id": request.manifest.npu_id, "layer": layer,
                    "activation_ms": now_ms, "deadline_ms": deadline_ms,
                    "release_ms": release, "delay_ms": delay,
    ...
    with patch.object(native._Context, "__init__", initialize), \
         ...
         patch.object(native, "_start_layer_io", start_io), \
