...
import render_fleet_cycle_ratio_ssu3 as fleet

base = fleet.base
plt = fleet.plt
ROOT, HERE = base.ROOT, base.HERE
OUT = HERE / 'figures/ssu3/per_npu'
CACHE = HERE / 'figures/ssu3/fleet_cycle_ratio/checks.json'
...
def single(order,npu,data):
    ...
    fig.text(.065,.930,f'32 NPU / 3 SSU × 40 GiB/s · seed 7 · warm [2,4) 秒 · 本卡真实 U={lane["U_percent"]:.2f}% · 整机真实 U={data["U_percent"]:.2f}%',fontsize=12,color=MUTED)
...
def paired(npu,data):
    ...
    fig.text(.065,.928,'32 NPU / 3 SSU × 40 GiB/s · seed 7 · warm [2,4) 秒 · 本图为 Random / Ordered 对照；分开的 PNG 在同目录',fontsize=12,color=MUTED)
...
def main():
    ...
    example=data['ordered']['lanes'][15]['zooms']['A'][0]
    md=['# 逐卡带宽：按完整内部层周期平均的 b_i','',
        '32 NPU、3 SSU × 40 GiB/s、seed 7、同一warm [2,4) 秒。使用既有物理接收日志的逐层积分结果，没有新增仿真。','',
        ...
        f'例如本次3盘Ordered NPU15的一个A内部周期：收到{example["V_GiB"]*1024:.5f}MiB，计算{example["C_ms"]:.3f}ms，等待{example["D_ms"]-example["C_ms"]:.3f}ms，完整周期{example["D_ms"]:.3f}ms。因此蓝线画为{example["mean_b_GiB_s"]:.6f}GiB/s，紫线仍为{example["B_GiB_s"]:.6f}GiB/s。没有把实际50GiB/s的短暂传输画成密集尖峰，也没有只对传输中的时间取平均。','',
    ...
    md+=['','[完整周期定义与整机汇总](../fleet_cycle_ratio/README.md) · [逐周期数据和来源校验](../fleet_cycle_ratio/checks.json) · [本次生成校验](layer_average_checks.json) · [生成代码](../../../render_per_npu_layer_avg_ssu3.py)','']
