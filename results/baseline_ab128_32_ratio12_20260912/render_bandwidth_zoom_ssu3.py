#!/usr/bin/env python3
"""3-SSU per-card A-to-B bandwidth zooms, PNG only, from existing logs."""
...
ROOT = HERE.parents[1]
OUT = HERE/'figures/ssu3/per_npu_zoom'
...
def draw(order,npu,lane,reqs,man,rows):
    ...
    assert left <= release < deadline < right and ready <= right
    ...
    for disk in range(3):
        disk_rows=rows[rows[:,4]==disk]
        disk_curve=curve(disk_rows,9,10,40,left,right)
        assert disk_curve[1].max()<=40+1e-8
    ...
    close(next_layer['io_barrier_wait_ms'],max(0.,-hidden))
    next_ready_relative=next_layer['io_ready_time_ms']-release
    if hidden > 1e-8:
        next_caption=f'本次 B 第2层提前 {hidden:.3f} ms 到齐，随后可用已备数据计算。'
    elif hidden < -1e-8:
        next_caption=f'本次 B 第2层晚到 {-hidden:.3f} ms，还会产生相应 IO 等待。'
    else:
        next_caption='本次 B 第2层恰在计算截止时到齐，无新增 IO 等待。'
    if next_ready_relative > 65:
        next_caption+='该到齐时刻位于图右侧之外。'
    ...
    fig.text(.065,.937,'32 NPU / 3 SSU × 40 GiB/s · NPU 接收上限 50 GiB/s · seed 7 · 逐块真实服务，无平滑',fontsize=13,color='#53677e')
    ...
    ax_s.set(ylim=(0,140),yticks=[0,40,80,120],ylabel='GiB/s')
    ax_s.set_title('② 盘端读出：橙线是三盘为本卡服务的合计速率；读出后还需经过 NPU 接收链路',loc='left',fontsize=14,pad=14,color=INK)
    ax_s.text(.22,.64,'橙线最多 120，蓝线最多 50；两图纵轴不同，比较时请看刻度。',transform=ax_s.transAxes,
    ...
    fig.text(.065,.052,next_caption+'瞬时 b_i(t)/B_i 不是瞬时利用率。',fontsize=12,color=INK)
    ...
    plt.close(fig)
    return dict(order=order,npu=npu,seed=7,num_ssu=3,num_npu=32,candidate_count=count,
        ...
        next_layer_early_ready_ms=hidden,next_layer_ready_relative_ms=next_ready_relative,
        next_layer_stall_ms=next_layer['io_barrier_wait_ms'],local_npu_U_percent=100*compute/(right-left),
        ssd_GiB=ssd[2][-1],received_GiB=link[2][-1],rate_peaks_GiB_s={'ssd':float(ssd[1].max()),'npu':float(link[1].max())},
        byte_conservation=True,full_target_block_identity_verified=True,per_disk_capacity_checked=True,
        target_read_fully_inside_window=True,visible_labels_inside_canvas=True,
...
def main():
    ...
    for order in ('random','ordered'):
        ...
        assert man['metadata']['num_ssu']==3
        arr=np.asarray(trace.pop('rows'),dtype=float)
        assert np.array_equal(np.unique(arr[:,4]),np.arange(3))
    ...
    npu15={row['order']:row for row in outputs if row['npu']==15}
    lines=['# 3 SSU：每张 NPU 的带宽局部放大（只生成 PNG）','',
        ...
        f'推荐先看 NPU 15：Random 本次首层等待 {npu15["random"]["stall_ms"]:.3f} ms，Ordered 等待 {npu15["ordered"]["stall_ms"]:.3f} ms。它们是各自挑出的局部事件，不是同一绝对时刻，也不代表整体优劣。逐块服务重建、无分箱或平滑，没有运行新仿真。','',
    ...
    lines+=['','[来源与数值校验](checks.json) · [生成代码](../../../render_bandwidth_zoom_ssu3.py)','']
