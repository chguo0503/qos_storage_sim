...
OUT=HERE/'figures/ssu4/internal_layer_zoom'
...
def draw(order,npu,role,lane,reqs,man,rows):
    ...
    fig.text(.065,.935,f'32 NPU / 4 SSU × 40 GiB/s · seed 7 · 请求 {rid} · 只看第 2～4 层计算和第 3～5 层预取',fontsize=13,color='#53677e')
    ...
    ax_s.set_title('② SSD 读出：四盘为本卡服务的速率之和；它与上方 NPU 接收的纵轴刻度不同',loc='left',fontsize=14,pad=14,color=INK)
    ...
    return dict(order=order,role=role,npu=npu,request_id=rid,candidate_count=count,
...
def main():
    ...
    audit=dict(no_new_simulation=True,formats_created=['png'],sources=base.SOURCES,
        ...
        all_checks_passed=True,images=results)
    ...
    lines+=['','[数据和来源校验](checks.json) · [生成代码](../../../render_internal_layer_zoom.py)','']
