#!/usr/bin/env python3
"""Causal SLO routing experiment; preserves the existing FIFO/storage engine."""
...
def upper_budget(context, state, now_ms):
    """Only host-visible metadata/progress; no SSD queues or future events."""
    req = context.requests[state.request_id]
    if not req.admitted:
        # Cross-request prefetch has no admission-clock deadline yet.
        return None
    assert req.batch_size == 1
    batch = context.microbatches[req.batch_id]
    c = req.per_layer_compute_ms
    if batch.compute_active_layer >= 0:
        k = batch.compute_active_layer
        # Predict using observed start + fixed input compute profile, not
        # the simulator's scheduled completion event or hidden IO progress.
    ...
    deadline = req.admission_time_ms + 1.5*context.n_layers*c
    ...
    for s, size in layout:
        work[s] += size
    return RequestBudget(remaining_slo_ms=deadline-now_ms,
                         remaining_compute_ms=remaining_compute,
...
def main():
    ...
    ap.add_argument('--policy', choices=('baseline','once','mild','aggressive'), required=True)
    ...
    record = dict(status='running', strategy=args.policy, seed=args.seed,
        ...
        information='host request admission/progress/profile + original 5ms count-only disk snapshot',
        slo_clock='admission; unadmitted cross-request prefetch uses exact Once',
        parameters=dict(slack_budget_threshold_ms=6.,slack_paths_per_group=2 if args.policy=='mild' else 1),
    ...
    try:
        with patch.object(native,'_register_complete',observe), patch.object(shared,'shared_path_adapter',manager):
            result = original.run_case(requests,metadata,
                strategy='baseline' if args.policy=='baseline' else 'once',
