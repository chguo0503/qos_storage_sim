...
[Random 全32卡合图 PNG](../random_all_32npu_layer_average.png) · [Ordered 全32卡合图 PNG](../ordered_all_32npu_layer_average.png)：每张卡仅取原图最上方的带宽面板，32行共享时间轴和纵轴尺度。

合图每行左侧显示该卡在完整 warm `[2,4)` 秒内的平均利用率 `U`，按实际计算时间除以 2 秒统计。

合图右侧另标每卡整窗平均带宽需求和实际供给，包含灰区；[完整表格与统计定义](../warm_bandwidth_means.md)。
...
例如本次3盘Ordered NPU15的一个A内部周期：收到175.65625MiB，计算6.024ms，等待39.718ms，完整周期45.742ms。因此蓝线画为3.750115GiB/s，紫线仍为28.475926GiB/s。没有把实际50GiB/s的短暂传输画成密集尖峰，也没有只对传输中的时间取平均。
...
[完整周期定义与整机汇总](../fleet_cycle_ratio/README.md) · [逐周期数据和来源校验](../fleet_cycle_ratio/checks.json) · [本次生成校验](layer_average_checks.json) · [生成代码](../../../render_per_npu_layer_avg_ssu3.py)
