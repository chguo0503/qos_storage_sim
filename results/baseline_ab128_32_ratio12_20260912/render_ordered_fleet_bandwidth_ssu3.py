...
PNG = OUT/'ordered_all_32npu_layer_average.png'
...
def main():
    ...
    for row in audit['images']:
        path = ROOT/row['path']
        if path.name.startswith('ordered_npu_'):
            assert source.base.sha(path) == row['sha256']
            originals[path.name] = row
    assert set(originals) == {f'ordered_npu_{n:02d}.png' for n in range(32)}
    _, datasets = source.verified_data()
    data = datasets['ordered']
    averages = means.load_verified('ordered')
    totals = averages['totals']
    assert len(averages['rows']) == 32
    source.base.close(data['U_percent'], audit['U_percent']['ordered'])
    ...
    fig.text(.045,.986,'Baseline Ordered：32 张 NPU 的每层平均带宽与需求',
    ...
    for npu, ax in enumerate(axes):
        ...
        rows.append(dict(npu=npu,U_percent=lane['U_percent'],U_label=f'U={lane["U_percent"]:.2f}%',
                         source_png=originals[f'ordered_npu_{npu:02d}.png'],
    ...
    check = dict(all_checks_passed=True,no_new_simulation=True,format='png',
                 num_npu=32,num_ssu=3,order='ordered',seed=7,window_ms=[2000.,4000.],
