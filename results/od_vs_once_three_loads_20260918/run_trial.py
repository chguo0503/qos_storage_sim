#!/usr/bin/env python3
"""Run the exact 640-request population with the original full-pool Once.
...
PROFILE_KEYS = tuple((length, miss) for length in (32, 64, 80, 128, 160) for miss in (2048, 4096))
...
def main():
    ...
    assert meta["order"] in ("random", "ordered")
    ...
    for npu in range(NUM_NPU):
        counts = Counter((q.load['seq_len_k'], q.load['nql']) for q in requests if q.npu_id == npu)
        assert counts == Counter({key: 2 for key in PROFILE_KEYS}), (npu, counts)
    ...
    scenario = meta.get("scenario_candidate", "document")
    ...
    record = dict(status="running", completed_simulation=False, policy=args.policy,
                  ...
                  experiment_conditions=dict(num_npu=NUM_NPU, num_ssu=NUM_SSU, disk_GiB_s=DISK_BW,
                      ...
                      original_compute_and_volume=True, admission_reference='8 * own per-layer compute',
    ...
    try:
        ...
        if ownership['applicable']:
            assert ownership_counts == {(d, n, ownership['npu_path_ids'][n]): block_counts[d][n]
                                        for d in range(NUM_SSU) for n in range(NUM_NPU)}
        if args.policy == 'once':
        ...
        result.update(experiment="od_vs_once_three_loads_20260918", strategy=args.policy,
