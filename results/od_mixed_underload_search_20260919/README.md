# 每卡都混合长短请求，OD 在欠载时能有多差？

本轮本机与远端并行探索已完成。**找到了逐盘欠载时的 warm 退化，但没有找到长期保持八十几、或相差 10 个百分点的可靠案例。**

较强的完整实验中，OD 在 `[2,4)` 秒的平均 NPU 利用率为 **94.06%**，Once 为 **99.50%**；SLO×1.5 分别为 **73.75% / 100.00%**。同一份输入，warm 两策略均逐盘欠载，32 张卡一直活跃、每张卡都真实计算过 A 和 B。但 OD 到 `[8,12)` 秒已恢复到 **99.38%**，因此不能把前面的低值叫作长期状态。
...
|---|---:|---:|---:|---|
| 较强候选 / OD | 94.0580% | 73.7530% (621/842) | 37.7529 | 通过 |
| 较强候选 / Once | 99.4978% | 100.0000% (884/884) | 37.7529 | 通过 |
| 保守候选 / OD | 97.2811% | 96.3724% (797/827) | 37.8586 | 通过 |
| 保守候选 / Once | 99.5461% | 100.0000% (883/883) | 37.8586 | 通过 |

表内每个 OD/Once 配对均使用相同冻结 manifest。较强候选提升 **5.4398 个百分点的 U**、**26.2470 个百分点的 SLO 达标率**。它和保守候选是不同计算参数，不能交叉比较来表示策略收益。
...
|---|---:|---:|---|---|
| [2,4) | 94.0580% | 99.4978% | 通过 | 通过 |
| [2,6) | 96.0528% | 99.4552% | 通过 | 通过 |
| [4,8) | 98.2892% | 99.2873% | 通过 | 通过 |
| [8,12) | 99.3782% | 99.4279% | 通过 | 通过 |
| [12,16) | 99.4821% | 99.4828% | 通过 | 通过 |

OD 整个完整运行都逐盘欠载。Once 的完整运行也逐盘欠载。不同策略改变完成进度，会改变同一时刻正在运行的请求组合，因此逐盘欠载必须分别核验，不能只查一次输入配比。

OD 从启动到最后排空的 U 为 92.6250%，其中包含部分卡提前完成后的空闲；**这个较低的全程平均不能用来声称稳态很差**。完整相同请求人口的 SLO 为 OD 94.7804%、Once 100.0000%；它与 warm 接纳请求子集的 SLO 是不同统计。
...
这两种计算时间是合成参数，不是 data 原始测量。另有未修改原始 data 的正式测试；其中满足 warm 欠载的 OD U 在 97.97%–99.14% 之间，也未得到长期很低的 U。
...
|---|---:|---:|---:|---:|
| A | 76.9761% | 99.0104% | 71.2987% | 100.0000% |
| B | 99.4735% | 99.6171% | 100.0000% | 100.0000% |
...
本轮完整排空 **25 次**；原仿真 4 秒筛选 **61 次、60 种输入**，另有 1 次截断测量校验；连续模型仅用于候选筛选，没有把模型的低值冒充正式结果。
