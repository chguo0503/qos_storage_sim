...
TABLE=ast.literal_eval(DATA_TEXT)
DATA_SOURCE_SHA256=hashlib.sha256(DATA_TEXT.encode()).hexdigest()
HIT=127744
BASE_MISS=3328
...
def base_input(cycles,unique_prefix=False):
 requests=[]
 for n in range(32):
  for k in range(cycles):
   for pos,miss in enumerate((256,BASE_MISS,BASE_MISS)):
    rid=n*1000000+k*3+pos;prefix=rid if unique_prefix else n*1000000+pos
    ...
    load=dict(request_id=rid,npu_id=n,original_request_id=prefix,physical_prefix_id=prefix,
     ...
     compute_interpolation_anchors=anchors,constructed_profile=pos!=0,physical_prefix_reused=not unique_prefix,
...
def plan(name,cycles,period,unique_prefix=False):
 directory=HERE/'planning'/name;directory.mkdir(parents=True,exist_ok=False)
 assert math.isfinite(period) and period>0
 before=hashes();start=time.perf_counter();requests=base_input(cycles,unique_prefix);frozen={};records=[]
 artifacts=[Path(__file__),ROOT/'data',ROOT/'inputs/manifest.py',HERE.parent/'transition/metrics.py']
 artifact_before={str(p.relative_to(ROOT)):hashlib.sha256(p.read_bytes()).hexdigest() for p in artifacts}
 assert artifact_before['data']==DATA_SOURCE_SHA256
 ...
 def choose(ctx,n,t):
  ...
  if batch is not None and npu.compute_active is None and batch.compute_done_up_to==-1 and all(ctx.requests[r].io_ready[0] for r in batch.member_request_ids):
   ...
   if q.load['position']==2:
    ...
    if len(records)%32==0:
     progress=dict(tails_planned=len(records),cycle=q.load['cycle'],simulation_ms=t,wall_seconds=time.perf_counter()-start)
 ...
 assert artifact_before=={str(p.relative_to(ROOT)):hashlib.sha256(p.read_bytes()).hexdigest() for p in artifacts}
 ...
 meta=dict(name=name,cycles=cycles,num_npu=32,num_ssu=3,n_layers=8,sequence='ABB repeated',all_arrivals_ms=0,
  ...
  synthetic_compute=True,input_mode='interpolated',OD_tailored=True,fixed_B_hit_tokens=HIT,base_B_miss=BASE_MISS,quantization_end_bound_ms=QUANTIZATION_END_BOUND_MS,physical_prefix_reuse=not unique_prefix,
  physical_prefix_rule=('unique logical request ID; original native RingHash; no ID search' if unique_prefix else 'npu*1000000+position; original native RingHash; no ID search'),
  runtime_logical_id_rule='npu*1000000+cycle*3+position; unique',
  od_queue_depth_per_ssu=8192,planning_source_sha256=before,planning_artifact_sha256=artifact_before,
 ...
 write_json(directory/'command.json',dict(status='complete',kind='PLANNING_ONLY_NOT_EXPERIMENT',name=name,cycles=cycles,period_ms=period,
  source_sha256=before,planning_artifact_sha256=artifact_before,source_unchanged=True,planning_artifacts_unchanged=True,hook_restored=True,wall_seconds=time.perf_counter()-start,conditions_passed=valid,
...
def main():
 ...
 p.add_argument('--cycles',type=int,default=2);p.add_argument('--unique-prefix',action='store_true');p.add_argument('--period-ms',type=float,default=1650.);p.add_argument('--strategy',choices=['od_baseline','once'],default='od_baseline');a=p.parse_args()
 if a.mode in ('plan','both'):plan(a.name,a.cycles,a.period_ms,a.unique_prefix)
