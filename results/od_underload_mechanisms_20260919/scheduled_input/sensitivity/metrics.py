...
def live_summary(context):
    ...
    batches = [dict(npu_id=b.npu_id, member_request_ids=list(b.member_request_ids),
                    ...
                    layer_metrics=[dict(compute_start_ms=m.compute_start_ms,
                                        ...
                                        layer=m.layer, io_barrier_wait_ms=m.io_barrier_wait_ms)
...
def summarize(summary, manifests, left, right, *, full=False):
    ...
    return dict(start_ms=left, end_ms=right, warm_start_s=None if full else left/1000,
                ...
                role_and_stall=role_and_stall_summary(summary, manifests, left, right),
...
def role_and_stall_summary(summary, manifests, left, right):
    """Separate exposed wait intervals from compute, including clipped edges.

    Native io_barrier_wait_ms is compute_start minus previous compute_end
    (or request admission for L0), so it is an NPU wait interval, not an IO
    service/queue latency. Each interval is assigned to its waiting request.
    """
    byid = {q.request_id:q for q in manifests}
    first = {}
    for q in manifests:
        first.setdefault(q.npu_id, q.request_id)
    roles = sorted({q.load['role'] for q in manifests})
    role_compute = [{role:0.0 for role in roles} for _ in range(32)]
    role_active = [{role:0.0 for role in roles} for _ in range(32)]
    computed_requests = [{role:set() for role in roles} for _ in range(32)]
    kinds = ('initial_L0', 'cross_request_L0', 'internal_L1_to_L7')
    totals = {kind:0.0 for kind in kinds}
    counts = {kind:0 for kind in kinds}
    per_npu = [{kind:0.0 for kind in kinds} for _ in range(32)]
    by_role = {role:{kind:0.0 for kind in kinds} for role in roles}
    events = []
    for row in summary['request_metrics']:
        q = byid[row['request_id']]
        role_active[q.npu_id][q.load['role']] += overlap(row['admission_time_ms'], row['completion_time_ms'], left, right)
    for batch in summary['microbatch_metrics']:
        q = byid[batch['member_request_ids'][0]]
        n, role = q.npu_id, q.load['role']
        for layer in batch['layer_metrics']:
            start, end = layer['compute_start_ms'], layer['compute_end_ms']
            if not math.isfinite(start):
                continue
            duration = overlap(start, end, left, right)
            role_compute[n][role] += duration
            if duration > 0:
                computed_requests[n][role].add(q.request_id)
            stall = layer['io_barrier_wait_ms']
            a, z = max(left, start-stall), min(right, start)
            if z <= a:
                continue
            kind = ('internal_L1_to_L7' if layer['layer'] > 0 else
                    'initial_L0' if q.request_id == first[n] else 'cross_request_L0')
            totals[kind] += z-a; counts[kind] += 1
            per_npu[n][kind] += z-a; by_role[role][kind] += z-a
            events.append(dict(npu_id=n, request_id=q.request_id, role=role, layer=layer['layer'],
                kind=kind, start_ms=a, end_ms=z, duration_ms=z-a,
                original_start_ms=start-stall, original_end_ms=start))
    balance = [math.fsum(role_active[n].values())-math.fsum(role_compute[n].values())-
               math.fsum(per_npu[n].values()) for n in range(32)]
    assert max(abs(v) for v in balance) < 1e-6, ('compute/stall/active accounting', balance)
    both = [all(role_compute[n].get(role,0) > 0 for role in ('A','B')) for n in range(32)]
    return dict(per_npu_role_compute_ms=role_compute, per_npu_role_active_ms=role_active,
        per_npu_role_computed_request_count=[{r:len(ids) for r,ids in row.items()} for row in computed_requests],
        per_npu_has_A_and_B_compute=both, all_npus_have_A_and_B_compute=all(both),
        npus_with_A_and_B_compute=sum(both),
        io_stall=dict(total_card_ms=math.fsum(totals.values()), by_kind_card_ms=totals,
            intervals_by_kind=counts, per_npu_by_kind_ms=per_npu, by_role_kind_ms=by_role,
            intervals=sorted(events,key=lambda e:(e['start_ms'],e['npu_id'],e['layer'])),
            maximum_accounting_error_ms=max(abs(v) for v in balance),
            definition='Exposed NPU wait only; intersect [compute_start - io_barrier_wait, compute_start) with window'))
