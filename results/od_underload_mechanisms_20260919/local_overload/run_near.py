...
def pilot_metrics(ctx,requests):
 s=live_summary(ctx);roles=[dict(A=0.,B=0.) for _ in range(32)];active=[0.]*32;byid={q.request_id:q for q in requests}
 for r in s['request_metrics']:active[r['npu_id']]+=overlap(r['admission_time_ms'],r['completion_time_ms'],2000,4000)
 for b in s['microbatch_metrics']:
  role=byid[b['member_request_ids'][0]].load['role']
  for l in b['layer_metrics']:roles[b['npu_id']][role]+=overlap(l['compute_start_ms'],l['compute_end_ms'],2000,4000)
 return dict(U_percent=100*sum(sum(r.values()) for r in roles)/64000.,all_npus_active=all(abs(x-2000)<1e-7 for x in active),per_npu_role_compute_ms=roles,mixed_cards=sum(min(r.values())>0 for r in roles),demand=exact_demand(s['request_metrics'],byid,2000,4000),observed_at_ms=ctx.current_time_ms,SLO_not_computed=True,scientific_status='4s_pilot_not_full_drain')
...
def main():
 ...
 busy=[[0.]*3 for _ in WINDOWS];warm_bins=[[0.]*200 for _ in range(3)];per_npu=[[0.]*32 for _ in range(3)];saved_preview=False
 ...
 try:
  ...
  assert all(r['all_npus_active'] for r in analyses[:3])
  # Original finite population may drain near16s; keep the last-window flag explicit.
