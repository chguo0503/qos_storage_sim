...
from inputs.runners.baseline_path0_layout import ROOT, RESULT
from simulator.core import continuous_batch_sim as batch_sim
...
def _placement(profile: dict, request_id: int) -> tuple[tuple[int, float], ...]:
    ...
    return tuple(
        (
            sim.block_ring_hash_disk_id(request_id, block_idx, 1),
            float(
...
def _build_requests(profiles: Sequence[dict]) -> tuple[ContinuousBatchRequest, ...]:
    ...
    for profile, request_count in zip(profiles, counts):
        ...
        for generation in range(request_count):
            ...
            placement = _placement(profile, request_id)
            load = {
...
def run_search(output: Path, jobs: int) -> list[dict]:
    ...
    _atomic_csv(data_dir / "search_summary.csv", fields, rows)
    valid = [row for row in rows if row["status"] == "valid"]
    winner = min(valid, key=lambda row: row["mean_npu_utilization"])
    _atomic_json(
        data_dir / "search_result.json",
        {
            "schema_version": 1,
            "scope": {
...
def run_result(output: Path, *, skip_counterfactual=False) -> dict:
    data_dir, figures_dir, docs_dir = (output / name for name in ("data", "figures", "docs"))
    ...
    _atomic_csv(data_dir / "input_profiles.csv", tuple(profile_rows[0]), profile_rows)
    layer_rows = sorted(
        trace.layer_rows,
        key=lambda row: (row["compute_start_time_ms"], row["npu_id"], row["layer"]),
    )
    _atomic_csv(data_dir / "request_layer_timeline.csv", tuple(layer_rows[0]), layer_rows)
    block_rows = sorted(trace.block_rows, key=lambda row: row["ssd_service_order"])
    _atomic_csv(data_dir / "physical_block_trace.csv", tuple(block_rows[0]), block_rows)
    _atomic_json(data_dir / "simulator_summary.json", baseline)

    _plot_npu_timeline(layer_rows, baseline, figures_dir / "01_npu_io_compute_timeline.png")
    _plot_enqueue_order(block_rows, figures_dir / "02_ssu_path0_enqueue_order.png")
    _plot_service(block_rows, baseline, figures_dir / "03_ssu_path0_service_timeline.png")
    _plot_blocks(baseline, figures_dir / "04_100ms_npu_utilization.png")

    search_path = data_dir / "search_result.json"
    bounded_search = (
    ...
    _atomic_json(data_dir / "result.json", result)
    if not skip_counterfactual:
        _atomic_text(docs_dir / "report.md", _report(result))
    print(
...
def main() -> None:
    ...
    parser.add_argument("--output", type=Path, default=DEFAULT_OUTPUT,
                        help="Result root; writes supporting files under data/, figures/, and docs/")
