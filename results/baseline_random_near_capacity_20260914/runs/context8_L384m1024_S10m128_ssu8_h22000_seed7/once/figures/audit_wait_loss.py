...
def main():
    ...
    assert raw['strategy'] == command['strategy'] == 'once'
    ...
    for left,right in ((2000.,4000.),(2000.,20000.)):
        ...
        for npu,lane in lanes.items():
            ...
            for batch in sorted(lane,key=lambda b:b['admission_time_ms']):
                ...
                if prior is not None:
                    ...
                    # This completed run uses the previous final computation as
        ...
        idle_raw=denom-active
        assert idle_raw >= -1e-6
        idle=0. if abs(idle_raw)<1e-6 else idle_raw
        ...
        results.append(dict(start_ms=left,end_ms=right,U_percent=100*compute/denom,
            ...
            denominator_card_ms=denom,idle_before_roundoff_cleanup_card_ms=idle_raw,
            groups=groups,transitions=dict(transitions),
            all_adjacent_request_prefetch_release_times_verified=release_count))
    record=dict(all_checks_passed=True,no_new_simulation=True,no_analyzer_import=True,strategy="once",
        ...
        link_lower_bound_formula='max(0, V_next/50GiBps - C_previous_last_layer), given the verified prefetch release at previous final compute start; each exposed interval is clipped to the window.',
    ...
    lines=['# 384K流量分配策略：把内部层等待和首层等待分开','',
           '32 NPU、8 SSU×40 GiB/s；长短请求 C 均为外推。理想平均负载99.69%，不代表逐盘逐时欠载。以下直接重读原始层日志并裁剪窗口，未导入已有分析器。','',
    ...
    lines+=['','前四项损失合计等于100%−U。最后一列包含在首层损失内，不能再加一次。','',
            '短请求最后一层只算0.716653 ms，而下一长请求首层需要接收526.625 MiB；即使数据源没有排队，50 GiB/s链路也至少需要10.285645 ms。因此按当前预取起点，S→L首层至少暴露9.568992 ms等待。这一部分不能归因于FIFO。','',
