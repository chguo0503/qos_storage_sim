#!/usr/bin/env python3
"""Once same-request cycle paired with the existing Baseline example when traced.
...
REFERENCE_RID, REFERENCE_NPU, REFERENCE_LAYER = 12000055, 12, 5
...
def choose_paired(raw, requests, short_roles, warm, trace_window):
    complete = []
    for batch in raw['summary']['microbatch_metrics']:
        rid = batch['member_request_ids'][0]
        if requests[rid]['load']['role'] not in short_roles:
            continue
        for current, following in zip(batch['layer_metrics'], batch['layer_metrics'][1:]):
            if warm[0] <= current['compute_start_ms'] < following['compute_start_ms'] <= warm[1]:
                complete.append(dict(request_id=rid, npu=batch['npu_id'], current=current, following=following,
                                     wait_ms=following['compute_start_ms']-current['compute_end_ms']))
    positive_internal = [r for r in complete if r['wait_ms'] > audit.TOL_MS
                         and r['current']['layer'] >= 1
                         and r['following']['layer'] < len(raw['summary']['microbatch_metrics'][0]['layer_metrics'])-1]
    median = statistics.median(r['wait_ms'] for r in positive_internal) if positive_internal else None
    selection = dict(warm_window_ms=list(warm), short_roles=list(short_roles),
        complete_short_cycles_including_zero_wait=len(complete),
        complete_short_cycles_positive_wait=sum(r['wait_ms'] > audit.TOL_MS for r in complete),
        eligible_cycles=len(positive_internal), positive_wait_median_ms=median,
        selected_distance_from_median_ms=None)
    reference = next(b for b in raw['summary']['microbatch_metrics']
                     if b['member_request_ids'] == [REFERENCE_RID])
    assert reference['npu_id'] == REFERENCE_NPU
    current, following = reference['layer_metrics'][REFERENCE_LAYER:REFERENCE_LAYER+2]
    assert current['layer'] == REFERENCE_LAYER and following['layer'] == REFERENCE_LAYER+1
    a, z = current['compute_start_ms'], following['compute_start_ms']
    paired = trace_window[0] <= a < z <= trace_window[1]
    selection.update(paired_to_baseline=paired,
        baseline_reference=dict(npu=REFERENCE_NPU, request_id=REFERENCE_RID, current_layer=REFERENCE_LAYER),
        reference_interval_ms=[a, z], reference_fully_inside_trace=paired,
        note='Same request identity/layer across strategies when available; absolute times may differ. A single selected cycle does not establish the fleet-wide strategy effect.')
    if paired:
        selected = dict(request_id=REFERENCE_RID, npu=REFERENCE_NPU, current=current, following=following,
                        wait_ms=z-current['compute_end_ms'])
        selection['rule'] = 'Use exactly the Baseline example request identity and internal layer, without selecting a new extremum; require the complete cycle inside the existing trace.'
    else:
        assert positive_internal, 'Reference not traced, and no eligible positive-wait fallback.'
        selected = min(positive_internal, key=lambda r:(abs(r['wait_ms']-median),r['request_id'],r['current']['layer']))
        selection['selected_distance_from_median_ms'] = abs(selected['wait_ms']-median)
        selection['rule'] = 'Baseline example outside this trace; use an unpaired warm complete internal short cycle nearest the median positive wait, ties by request_id and layer.'
    selection['selected_cycle_in_warm'] = warm[0] <= selected['current']['compute_start_ms'] < selected['following']['compute_start_ms'] <= warm[1]
...
def prepare(case, warm):
    ...
    assert raw['strategy'] == trace['strategy'] == 'once'
    meta = man['metadata']
    requests = {r['request_id']: r for r in man['requests']}
    request_metrics = {r['request_id']: r for r in raw['summary']['request_metrics']}
    selected, selection = choose_paired(raw, requests, analysis['short_roles'], warm, trace['window_ms'])
    ...
    audit.close(right, max(deadline, ready))
    ...
    assert np.all((rows[:, 5] >= 0) & (rows[:, 5] < 256) & (rows[:, 5] == np.floor(rows[:, 5]))) and np.all(rows[:, 7] == 1)
    ...
    # Multiple QoS paths: earlier physical service does not prove one FIFO order.
    ...
    evidence = dict(all_checks_passed=True, no_simulation=True, case=str(case), strategy=raw['strategy'],
                    ...
                    observed_paths=sorted(int(p) for p in np.unique(shown[:,5])), target_paths_by_ssu={str(d):sorted(int(p) for p in np.unique(target[target[:,4]==d,5])) for d in range(meta['num_ssu'])},
                    earlier_service_is_not_a_FIFO_order_claim=True,
...
def draw(data, output):
    ...
    fig.text(.06, .963, '流量分配策略 Random：同一短请求内部，计算与下一层 IO 如何重叠？', fontsize=24, color=INK)
    ...
    paired_label = '与Baseline相同请求、相同层；绝对时间可不同' if selection['paired_to_baseline'] else '未配对：原示例不在trace内，改取本策略warm正等待短层中位附近'
    fig.text(.06, .886, paired_label + ('；本周期在warm内。' if selection['selected_cycle_in_warm'] else '；本周期不在warm内，整机U仍按warm统计。'), fontsize=12.5, color=MUTED)
    ...
    for ax in (npu_ax, disk_ax, volume_ax):
        ...
        if abs(e['ready_ms']-left-D) > audit.TOL_MS:
            ax.axvline(e['ready_ms']-left, color=TEAL, ls=':', lw=1.4, zorder=6)
    ...
    if D-C > audit.TOL_MS:
        ...
        npu_ax.text(D, -.13, f'IO-ready +{e["ready_ms"]-left:.3f} ms；下层计算 +{D:.3f} ms', ha='right', va='top', color=COMPUTE, fontsize=12)
    else:
        npu_ax.text(D/2, -.13, f'数据 +{e["ready_ms"]-left:.3f} ms 已到齐；计算 +{D:.3f} ms 结束并开始下层；w = 0', ha='center', va='top', color=COMPUTE, fontsize=12)
    disk_ax.set_title('② SSU：多个 QoS path 汇总的实际磁盘服务；服务先后不等于单一 FIFO 排队', loc='left', fontsize=16, pad=16, color=INK)
    disk_ax.set(ylim=(-.55, meta['num_ssu']+.7), yticks=range(meta['num_ssu']),
                yticklabels=[f'SSU {s} / 多 path' for s in range(meta['num_ssu'])])
    ...
    missing_label = f'计算截止仍缺 {missing:.4f} MiB' if missing > 1e-9 else '计算截止时，下一层数据已全部到齐'
    volume_ax.annotate(missing_label, xy=(C, e['received_at_compute_deadline_MiB']),
    ...
    fig.text(.075, .142, f'实际服务：目标首次获得 SSD 服务在起点后 {first_ssd:.3f} 毫秒；此前各盘其他长请求的服务约 {min(longs):.3f}～{max(longs):.3f} 毫秒。', fontsize=13.5, color=INK)
    fig.text(.075, .115, f'计算在 +{C:.3f} ms 结束；此时收到 {e["received_at_compute_deadline_MiB"]:.4f}/{V:.4f} MiB；到 +{e["ready_ms"]-left:.3f} ms 收齐，等待 {D-C:.3f} ms。', fontsize=13.5, color=INK)
    ...
    fig.text(.075, .032, '配对仅表示相同输入身份与层号；两种策略到达该层时的其他卡状态可能不同。此局部样例不能替代整窗、整机对照。', fontsize=11.5, color=MUTED)
