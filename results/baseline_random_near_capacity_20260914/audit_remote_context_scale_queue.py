...
def main():
    plan_path = HERE / 'audit_remote_context_scale_plan.json'
    plan = read(plan_path)
    ...
    assert plan['max_context_scale_parallel'] == 4 and plan['remote_simulation_limit'] == 6
    ...
    assert sum(job['trace'] for job in plan['jobs']) == 1
    assert all(not job['trace'] or job['long_total_k'] == 512 for job in plan['jobs'])
    ...
    meta = {'host': platform.node(), 'python_version': sys.version, 'pid': os.getpid(),
            ...
            'launch_record_sha256': sha(HERE / 'audit_remote_context_scale_launch.json'),
            'max_context_scale_parallel': 4, 'remote_simulation_limit': 6,
            'source_hashes_verified_before': True, 'selection': plan['selection']}

    def publish():
        write(HERE / 'audit_remote_context_scale_status.json', dict(meta, updated_utc=utc(), jobs=state,
    ...
    while pending or active:
        ...
        while pending and predecessor_pending == 0 and len(active) < 4 and occupied + len(active) < 6:
            job = pending.pop(0)
            label = job['label']
            log = HERE / 'execution_logs' / (label + '.context_scale.remote.log')
            stream = log.open('w')
            ...
            state[label].update(status='running', pid=process.pid, started_utc=utc(), log=str(log),
                                ...
                                context_scale_running_after_start=len(active),
