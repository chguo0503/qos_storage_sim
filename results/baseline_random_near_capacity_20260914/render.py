...
def trace_evidence(case,man,raw,command,left,right):
    ...
    # The existing layer/block identity order also groups requests. Reuse its
    # indices rather than scanning millions of rows again for every request.
    sorted_rids=rows[order,0].astype(np.int64)
    request_ids,starts=np.unique(sorted_rids,return_index=True)
    ends=np.r_[starts[1:],len(order)]
    for rid,a,z in zip(request_ids,starts,ends):
        request=reqs[int(rid)]
        selected=rows[order[a:z]]
        assert np.all(selected[:,0]==rid)
...
def construction_notice(metadata):
    extrapolated={p['role'] for p in metadata['profiles']
                  if p.get('construction',{}).get('method','').startswith('affine_extrapolation')}
    if {'L','S'}<=extrapolated:return '长、短请求 C 均为外推'
    if 'S' in extrapolated:return '短请求 C 外推'
    if 'L' in extrapolated:return '长请求 C 外推'
    return '部分请求 C 外推' if extrapolated else ''
...
def configuration(data):
    ...
    config=f'{m["num_npu"]} NPU / {m["num_ssu"]} SSU × {m["disk_bw_gib_s"]:g} GiB/s · seed {m["seed"]} · warm [{data["left"]/1000:g},{data["right"]/1000:g}) 秒'
    notice=construction_notice(m)
    if notice:config+=' · '+notice
    return f'{strategy} Random · {candidate}',config
...
def history(data,out):
    ...
    notice=construction_notice(meta)
    subtitle=f'32 NPU / {meta["num_ssu"]} SSU × {meta["disk_bw_gib_s"]:g} GiB/s · seed {meta["seed"]}；每点使用同一32卡固定分母'
    if notice:subtitle+=' · '+notice
    fig.text(.06,.877,subtitle,fontsize=13,color=MUTED)
