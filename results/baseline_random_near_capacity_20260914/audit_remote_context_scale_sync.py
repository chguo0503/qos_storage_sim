#!/usr/bin/env python3
"""Import four context-scale Baseline cases, including one remotely audited trace."""
...
def sync(password):
    plan = read(HERE / 'audit_remote_context_scale_plan.json')
    setup = read(HERE / 'audit_remote_setup.json')
    record_path = HERE / 'audit_remote_context_scale_sync_record.json'
...
Q=json.loads((H/'audit_remote_context_scale_status.json').read_text());remaining=__REMAINING__
plan=json.loads((H/'audit_remote_context_scale_plan.json').read_text());jobs={j['label']:j for j in plan['jobs']}
...
if done:
 ...
 A=D/('audit_context_scale_sync_'+str(time.time_ns())+'.tar')
 with tarfile.open(A,'w') as t:
  for label in done:
   ...
   t.add(H/'execution_logs'/(label+'.context_scale.remote.log'),arcname='execution_logs/'+label+'.context_scale.remote.log',recursive=False)
...
'''.replace('__DIR__', repr(setup['remote_dir'])).replace('__REMAINING__', repr(remaining)).replace('__CHECKER_SHA__', repr(setup['trace_validator']['sha256']))
    ...
    assert queue['plan_sha256'] == sha(HERE / 'audit_remote_context_scale_plan.json')
    assert queue['queue_runner_sha256'] == sha(HERE / 'audit_remote_context_scale_queue.py')
    assert queue['launch_record_sha256'] == sha(HERE / 'audit_remote_context_scale_launch.json')
    launch = read(HERE / 'audit_remote_context_scale_launch.json')
    assert launch['construction_audit_passed'] and sha(HERE / launch['audit_report']) == launch['audit_report_sha256']
    (HERE / 'audit_remote_context_scale_status.json').write_text(json.dumps(queue, ensure_ascii=False, indent=2) + '\n')
    new = response['new_complete']
    if new:
        incoming = HERE / 'runtime_validation/remote_context_scale_incoming'
        ...
        expected.update('execution_logs/' + label + '.context_scale.remote.log' for label in new)
        ...
        for label in new:
            ...
            shutil.copyfile(batch / 'execution_logs' / (label + '.context_scale.remote.log'),
                            HERE / 'execution_logs' / (label + '.context_scale.remote.log'))
    ...
    if finished:
        ...
        acceptance = dict(passed=True, checked_utc=datetime.now(timezone.utc).isoformat(),
                          scope='Four context-volume model stress Baseline pilots; one trace. Extrapolated contexts are not hardware measurements.',
        ...
        (HERE / 'audit_remote_context_scale_acceptance.json').write_text(json.dumps(acceptance, ensure_ascii=False, indent=2, allow_nan=False) + '\n')
