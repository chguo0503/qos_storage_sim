#!/usr/bin/env python3
"""Independently verify final context manifests; do not import any builder."""
import ast
...
def main():
    ...
    prepared_path=HERE/'context_prepared_seed7.jsonl'
    records=[json.loads(line) for line in prepared_path.read_text().splitlines() if line.strip()]
    assert len(records)==4 and len(plan['selected_candidates'])==4
    ...
    for record in records:
        ...
        name=candidate['name'];label=f'{name}_ssu8_h22000_seed7'
        ...
        assert meta['num_npu']==32 and meta['num_ssu']==8 and meta['n_layers']==8 and meta['seed']==7
        ...
        assert meta['horizon_pure_compute_ms']==22000 and meta['measurement_window_ms']==[2000,4000]
        ...
        repeats=math.ceil(22000/cycle);population=[n*repeats for n in counts]
        ...
        for npu in range(32):
            ...
            random.Random(7+100003*npu).shuffle(expected_order)
            ...
            assert total_compute>=22000;near(total_compute,repeats*cycle)
            ...
            assert assignment['npu_id']==npu and assignment['shuffle_seed']==7+100003*npu
        ...
        audits.append(dict(candidate=name,manifest=str(path.relative_to(ROOT)),manifest_sha256=manifest_sha,
            ...
            checks=dict(raw_source_unchanged=True,constructed_C_independently_fit=True,constructed_source_TTFT_null=True,
                ...
                all_fixed_NPU_assignments_verified=True,all_NPU_pure_compute_at_least22s=True,
    ...
    out=dict(schema_version='independent-context-input-audit-v1',all_checks_passed=True,
        no_builder_imported=True,no_simulations_run=True,
        scope='All four final seed7 manifests from context_prepared_seed7.jsonl',
    ...
    (HERE/'context_input_audit.json').write_text(json.dumps(out,ensure_ascii=False,indent=2)+'\n')
