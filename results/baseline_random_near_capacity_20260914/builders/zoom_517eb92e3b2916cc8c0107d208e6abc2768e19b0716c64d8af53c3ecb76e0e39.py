...
def draw(data, output):
    ...
    fig.text(.06, .935, f'{meta["candidate"]} · {meta["num_npu"]} NPU / {meta["num_ssu"]} SSU × {meta["disk_bw_gib_s"]:g} GiB/s · seed {meta["seed"]} · NPU {npu:02d} · 请求 {rid}', fontsize=14, color=MUTED)
    ...
    fig.text(.075, .817, f'平均 b / B = C / D = {e["local_cycle_U_percent"]:.2f}%（本层周期利用率）；整机 warm [2,4) 秒 U = {e["fleet_warm_U_percent"]:.2f}%', fontsize=15, color=INK)
