#!/usr/bin/env python3
"""Import and validate four predeclared raw-data load110 Baseline confirmation seeds."""
...
def verify_one(job, directory, plan, setup, queue):
    ...
    assert metadata['candidate'] == 'reference_load110' and metadata['seed'] == job['seed']
    assert metadata['profile_keys'] == '128:256,32:4096' and metadata['count_ratio'] == [17,31]
...
def sync(password):
    plan = read(HERE / 'audit_remote_load110_confirmation_plan.json')
    setup = read(HERE / 'audit_remote_setup.json')
    record_path = HERE / 'audit_remote_load110_confirmation_sync_record.json'
...
Q=json.loads((H/'audit_remote_load110_confirmation_status.json').read_text());remaining=__REMAINING__
...
if done:
 A=D/('audit_load110_confirmation_sync_'+str(time.time_ns())+'.tar.gz')
 with tarfile.open(A,'w:gz') as t:
  for label in done:
   ...
   t.add(H/'execution_logs'/(label+'.load110_confirmation.remote.log'),arcname='execution_logs/'+label+'.load110_confirmation.remote.log',recursive=False)
...
'''.replace('__DIR__', repr(setup['remote_dir'])).replace('__REMAINING__', repr(remaining))
    ...
    assert queue['plan_sha256'] == sha(HERE / 'audit_remote_load110_confirmation_plan.json')
    assert queue['queue_runner_sha256'] == sha(HERE / 'audit_remote_load110_confirmation_queue.py')
    assert queue['launch_record_sha256'] == sha(HERE / 'audit_remote_load110_confirmation_launch.json')
    ...
    (HERE / 'audit_remote_load110_confirmation_status.json').write_text(json.dumps(queue, ensure_ascii=False, indent=2) + '\n')
    new = response['new_complete']
    if new:
        incoming = HERE / 'runtime_validation/remote_load110_confirmation_incoming'
        ...
        expected.update('execution_logs/' + label + '.load110_confirmation.remote.log' for label in new)
        ...
        for label in new:
            ...
            assert not target.exists(), f'Preserving existing load110_confirmation result: {target}'
            os.replace(batch / label, target)
            shutil.copyfile(batch / 'execution_logs' / (label + '.load110_confirmation.remote.log'),
                            HERE / 'execution_logs' / (label + '.load110_confirmation.remote.log'))
    ...
    if finished:
        ...
        acceptance = dict(passed=True, checked_utc=datetime.now(timezone.utc).isoformat(),
                          scope='Four predeclared Baseline Random confirmation seeds after seed7 load110 pilot; raw profiles with ideal mean load about 1.10, outside strict underload.',
        ...
        (HERE / 'audit_remote_load110_confirmation_acceptance.json').write_text(json.dumps(acceptance, ensure_ascii=False, indent=2, allow_nan=False) + '\n')
