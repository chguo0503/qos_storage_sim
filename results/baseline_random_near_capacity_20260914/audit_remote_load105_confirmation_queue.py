#!/usr/bin/env python3
"""Four predeclared raw-data load105 Baseline confirmation seeds, within the shared six-process limit."""
...
def main():
    ...
    assert plan['max_load105_confirmation_parallel'] == 4 and plan['remote_simulation_limit'] == 6
    ...
    meta = {'host': platform.node(), 'python_version': sys.version, 'pid': os.getpid(),
            ...
            'launch_record_sha256': sha(HERE / 'audit_remote_load105_confirmation_launch.json'),
            'max_load105_confirmation_parallel': 4, 'remote_simulation_limit': 6,
            'source_hashes_verified_before': True, 'selection': plan['selection']}

    def publish():
        write(HERE / 'audit_remote_load105_confirmation_status.json', dict(meta, updated_utc=utc(), jobs=state,
    ...
    while pending or active:
        ...
        while pending and predecessor_pending == 0 and len(active) < 4 and occupied + len(active) < 6:
            job = pending.pop(0)
            label = job['label']
            log = HERE / 'execution_logs' / (label + '.load105_confirmation.remote.log')
            stream = log.open('w')
            ...
            state[label].update(status='running', pid=process.pid, started_utc=utc(), log=str(log),
                                ...
                                load105_confirmation_running_after_start=len(active),
