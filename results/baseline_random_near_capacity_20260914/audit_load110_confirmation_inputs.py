...
def main():
    ...
    for job in plan['jobs']:
        ...
        assert meta['candidate'] == 'reference_load110' and meta['seed'] == job['seed']
        ...
        for npu in range(32):
            ...
            assert assignment['role_counts'] == dict(Counter(roles)) == {'A':17*repeat,'B':31*repeat}
        ...
        audits[job['label']] = dict(manifest_sha256=sha(path),input_fingerprint=fp.hexdigest(),
            ...
            unique_role_queues=32,per_card_counts={'A':17*repeat,'B':31*repeat},
    ...
    report = dict(passed=True,checked_utc=datetime.now(timezone.utc).isoformat(),
        plan_sha256=sha(plan_path),auditor_sha256=sha(HERE/'audit_load110_confirmation_inputs.py'),
        seed_selection='19,43,67,101 fixed before these confirmation results; pilot seed7 selected load110 after load sensitivity results.',
        caveat='Ideal population mean demand is about 110% of 3-disk capacity. This is an overload confirmation, not a strict-underload FIFO counterexample.',
        cases=audits)
    (HERE/'audit_load110_confirmation_inputs.json').write_text(json.dumps(report,ensure_ascii=False,indent=2)+'\n')
