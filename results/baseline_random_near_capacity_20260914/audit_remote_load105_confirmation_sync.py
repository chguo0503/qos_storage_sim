#!/usr/bin/env python3
"""Import and validate four predeclared raw-data load105 Baseline confirmation seeds."""
...
def verify_one(job, directory, plan, setup, queue):
    ...
    assert metadata['family'] == 'raw' and metadata['constructed_profile'] is False
    assert metadata['candidate'] == 'reference_load105' and metadata['seed'] == job['seed']
    assert metadata['profile_keys'] == '128:256,32:4096' and metadata['count_ratio'] == [32,63]
    assert metadata['role_names'] == ['A','B'] and metadata['num_npu'] == 32 and metadata['n_layers'] == 8
    assert metadata['num_ssu'] == 3 and metadata['horizon_pure_compute_ms'] == 22000
    assert metadata['source_data_sha256'] == command['source_data_sha256']
    assert result['metadata']['constructed_profile'] is False
    for row in manifest['requests']:
        load = row['load']; role = load['role']
        assert role in ('A','B') and load['constructed_profile'] is False
        assert load['profile_construction']['method'] == 'direct_data_row'
        assert load['source_ttft_ms'] is not None
        assert (load['seq_len_k'],load['nql']) == ((128,256) if role == 'A' else (32,4096))
    return dict(command_sha256=sha(directory / 'command.json'), manifest_sha256=command['manifest_sha256'],
                ...
                all_hashes_blocks_and_invariants_verified=True, raw_data_profile_flags_verified=True)


def sync(password):
    plan = read(HERE / 'audit_remote_load105_confirmation_plan.json')
    setup = read(HERE / 'audit_remote_setup.json')
    record_path = HERE / 'audit_remote_load105_confirmation_sync_record.json'
...
Q=json.loads((H/'audit_remote_load105_confirmation_status.json').read_text());remaining=__REMAINING__
...
if done:
 A=D/('audit_load105_confirmation_sync_'+str(time.time_ns())+'.tar.gz')
 with tarfile.open(A,'w:gz') as t:
  for label in done:
   ...
   t.add(H/'execution_logs'/(label+'.load105_confirmation.remote.log'),arcname='execution_logs/'+label+'.load105_confirmation.remote.log',recursive=False)
...
'''.replace('__DIR__', repr(setup['remote_dir'])).replace('__REMAINING__', repr(remaining))
    ...
    assert queue['plan_sha256'] == sha(HERE / 'audit_remote_load105_confirmation_plan.json')
    assert queue['queue_runner_sha256'] == sha(HERE / 'audit_remote_load105_confirmation_queue.py')
    assert queue['launch_record_sha256'] == sha(HERE / 'audit_remote_load105_confirmation_launch.json')
    launch = read(HERE / 'audit_remote_load105_confirmation_launch.json')
    assert launch['input_audit_passed'] and sha(HERE / launch['audit_report']) == launch['audit_report_sha256']
    (HERE / 'audit_remote_load105_confirmation_status.json').write_text(json.dumps(queue, ensure_ascii=False, indent=2) + '\n')
    new = response['new_complete']
    if new:
        incoming = HERE / 'runtime_validation/remote_load105_confirmation_incoming'
        ...
        expected.update('execution_logs/' + label + '.load105_confirmation.remote.log' for label in new)
        ...
        for label in new:
            ...
            assert not target.exists(), f'Preserving existing load105_confirmation result: {target}'
            os.replace(batch / label, target)
            shutil.copyfile(batch / 'execution_logs' / (label + '.load105_confirmation.remote.log'),
                            HERE / 'execution_logs' / (label + '.load105_confirmation.remote.log'))
    ...
    if finished:
        ...
        acceptance = dict(passed=True, checked_utc=datetime.now(timezone.utc).isoformat(),
                          scope='Four predeclared Baseline Random confirmation seeds after seed7 load105 pilot; raw profiles with ideal mean load about 1.05, outside strict underload.',
        ...
        (HERE / 'audit_remote_load105_confirmation_acceptance.json').write_text(json.dumps(acceptance, ensure_ascii=False, indent=2, allow_nan=False) + '\n')
