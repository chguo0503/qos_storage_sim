#!/usr/bin/env python3
"""Prepare low-bandwidth-short model stress inputs, without simulation.

Raw 200K/miss1024 is preserved. Longer contexts and 10K/miss1024 are
...
def prepare(spec, plan, seed, horizon=22000.):
    ...
    assert int(spec['num_ssu']) == 4
    assert int(spec['long_total_k']) in (200, 256, 384, 512) and int(spec['long_nql']) == 1024
    assert int(spec['short_total_k']) == 10 and int(spec['short_nql']) == 1024
    ...
    assert short_model['fixed_miss'] == 1024 and long_model['fixed_miss'] == 1024
    ...
    if spec['long_is_raw']:
        ...
        assert long['per_layer_compute_us'] == spec['long_compute_us']
    else:
    ...
    assert counts == (1, {200: 16, 256: 21, 384: 32, 512: 43}[int(spec['long_total_k'])])
    assert spec['name'] == f"lowB4_L{int(spec['long_total_k'])}m1024_S10m1024"
    assert short['per_layer_compute_us'] < long['per_layer_compute_us']
    assert short['required_bandwidth_gibps'] < long['required_bandwidth_gibps']
    ...
    metadata = dict(experiment='baseline_random_near_capacity_20260914', label=label, case_id=label+'_'+fp[:12],
        ...
        disk_bw_gib_s=base.DISK_BW, npu_bw_gib_s=base.NPU_BW, family='data_affine_low_b_short',
        ...
        purpose='Separate low-bandwidth-short hypothesis: 4 SSU, S10K/miss1024 has smaller C and lower V/C than L200/256/384/512K/miss1024. Test absolute long-read burst scale near ideal mean capacity with an independent fully shuffled population; do not mix with the S10K/miss128 high-B context family.',
        caveat='200K/miss1024 long profile is an unchanged data row. Other long C and all 10K/miss1024 short C are affine extrapolations of fixed-miss1024 data, with source TTFT unset for constructed roles. This independent low-B-short family is not the prior S10K/miss128 high-B-short context family, real 256/384/512K hardware, or original-data performance. Ideal mean near capacity is not per-instant disk underload. Every card contains both roles, but actual warm mixed coverage is unguaranteed and requires post-run audit.')
    ...
    # An existing frozen input must keep its compressed bytes and SHA.
    if not path.exists():
