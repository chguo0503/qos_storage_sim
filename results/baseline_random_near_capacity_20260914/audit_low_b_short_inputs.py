#!/usr/bin/env python3
"""Independently verify final low-B-short manifests; do not import any builder."""
import argparse
import ast
...
def main():
    ...
    parser.add_argument('--prepared',type=Path,default=HERE/'low_b_short_prepared_seed7.jsonl')
    parser.add_argument('--seed',type=int,default=7)
    parser.add_argument('--horizon-ms',type=float,default=22000.)
    parser.add_argument('--allow-subset',action='store_true')
    parser.add_argument('--output',type=Path)
    args=parser.parse_args()
    seed,horizon=args.seed,args.horizon_ms
    assert math.isfinite(horizon) and horizon>=22000
    plan_path=HERE/'low_b_short_math.json'
    protected=[plan_path,HERE/'low_b_short_math.py',HERE/'prepare_low_b_short.py',HERE/'long_scale_math.json',
    ...
    records=[json.loads(line) for line in prepared_path.read_text().splitlines() if line.strip()]
    assert len(plan['selected_candidates'])==4
    assert (1<=len(records)<=4) if args.allow_subset else (len(records)==4)
    assert len({r['label'] for r in records})==len(records)
    by_name={c['name']:c for c in plan['selected_candidates']}
    fits={1024:fit_ms(raw,1024)}
    ...
    for record in records:
        ...
        name=candidate['name'];label=f'{name}_ssu4_h{horizon:g}_seed{seed}'
        assert meta['label']==record['label']==label and path.name==label+'.json.gz'
        assert meta['constructed_plan_sha256']==plan_sha
        assert Path(meta['constructed_plan_path']).resolve()==plan_path.resolve()
        assert meta['constructed_builder_sha256']==sha(HERE/'prepare_low_b_short.py')
        ...
        assert meta['num_npu']==32 and meta['num_ssu']==4 and meta['n_layers']==8 and meta['seed']==seed
        assert meta['disk_bw_gib_s']==40 and meta['npu_bw_gib_s']==50
        assert meta['order']==meta['order_mode']=='random'
        assert meta['family']=='data_affine_low_b_short' and meta['constructed_profile']
        ...
        for idx,role in enumerate(('L','S')):
            ...
            if is_raw:
                ...
                assert model['frozen_source_sha256']==sha(HERE/'long_scale_math.json')
        ...
        assert profiles[1]['B_GiB_s']<5<profiles[0]['B_GiB_s']<50
        assert profiles[1]['C_us']<profiles[0]['C_us']
        ...
        assert len(man['requests'])==len(canonical)*32==meta['request_count']==record['requests']
        ...
        checked_placement=set();role_sequences=set();disk_work=[0.]*4;full_compute=[]
        ...
        for npu in range(32):
            ...
            seen_roles=[];card_compute=[];card_work=[0.]*4
            for position,(r,original) in enumerate(zip(requests,expected_order)):
                ...
                assert load['category']==('LL' if idx==0 else 'SL')
                pi=r['placement_index'];placement=cached_placement[pi]
                key=(pi,npu%4,idx)
                if key not in checked_placement:
                    expected=(tuple(((j+npu)%4,BLOCK_GIB) for j in range(p['blocks'])),)
            ...
            for p,n in zip(profiles,population):
                for j in range(p['blocks']):card_work[(j+npu)%4]+=n*8*BLOCK_GIB
            for disk in range(4):disk_work[disk]+=card_work[disk]/(total_compute/1000)
        ...
        assert fingerprint==man['input_fingerprint']==meta['input_fingerprint']==record['input_fingerprint']
        ...
        assert len(role_sequences)==32 and blocks_total==record['expected_blocks']
        rho=math.fsum(disk_work)/160
        near(rho,meta['ideal_load_ratio']);near(rho,candidate['rho_ideal']);near(rho,record['ideal_load_ratio'])
        assert .95<=rho<=1.05
        ...
        audits.append(dict(candidate=name,manifest=str(path.relative_to(ROOT)),manifest_sha256=manifest_sha,
        ...
        print(json.dumps(dict(candidate=name,requests=len(all_sorted),rho=rho,pure_ms=min(full_compute),passed=True)),flush=True)
    assert before=={str(p.relative_to(ROOT)):sha(p) for p in protected}
    out=dict(schema_version='independent-low-B-short-input-audit-v1',all_checks_passed=True,
        no_builder_imported=True,no_simulations_run=True,
        scope=f'{len(records)} final low-B-short manifests, seed{seed}, pure-compute horizon{horizon:g}ms',seed=seed,horizon_pure_compute_ms=horizon,
    ...
    (args.output or HERE/f'low_b_short_input_audit_seed{seed}_h{horizon:g}.json').write_text(json.dumps(out,ensure_ascii=False,indent=2)+'\n')
