#!/usr/bin/env python3
"""Read completed context U/compute coverage before any large trace validation."""
...
def preview(password):
    setup = read(HERE / 'audit_remote_setup.json')
    path = HERE / 'audit_remote_context_preview.json'
...
P=json.loads((H/'audit_remote_context_scale_plan.json').read_text());jobs={j['label']:j for j in P['jobs']}
def read(p):
...
for label in remaining:
 ...
 p=H/'runs'/label/'baseline';c=read(p/'command.json')
 ...
 roles={x['request_id']:x['load']['role'] for x in m['requests']};windows=[]
 for a,z in [(2000.,4000.),(2000.,20000.)]:
  compute=[{'L':[],'S':[]} for _ in range(32)];active=[[] for _ in range(32)]
...
'''.replace('__DIR__', repr(setup['remote_dir'])).replace('__REMAINING__', repr(remaining))
    ...
    if response['new_cases']:
        ...
        record.update(updated_utc=datetime.now(timezone.utc).isoformat(),
                      preview_source_sha256=sha(HERE / 'audit_remote_context_preview.py'),
...
if __name__ == '__main__':
    import getpass
    preview(getpass.getpass('SSH password (not stored): '))
