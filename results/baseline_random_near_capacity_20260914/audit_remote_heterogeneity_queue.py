#!/usr/bin/env python3
"""Four context-volume scale Baseline inputs, within the shared six-process limit."""
...
def main():
    plan_path = HERE / 'audit_remote_heterogeneity_plan.json'
    plan = read(plan_path)
    launch = read(HERE / 'audit_remote_heterogeneity_launch.json')
    assert launch['plan_sha256'] == sha(plan_path) and launch['construction_audit_passed'] is True
    assert plan['max_heterogeneity_parallel'] == 3 and plan['remote_simulation_limit'] == 6
    assert len(plan['jobs']) == 3 and len({job['label'] for job in plan['jobs']}) == 3
    assert all(job['strategy'] == 'baseline' and job['order'] == 'random' for job in plan['jobs'])
    assert all(not job['trace'] and job['seed'] == 7 for job in plan['jobs'])
    assert plan['input_construction_finalized'] is True
    verify_sources(plan)
    ...
    meta = {'host': platform.node(), 'python_version': sys.version, 'pid': os.getpid(),
            ...
            'queue_runner_sha256': sha(Path(__file__)), 'job_count': 3,
            'launch_record_sha256': sha(HERE / 'audit_remote_heterogeneity_launch.json'),
            'max_heterogeneity_parallel': 3, 'remote_simulation_limit': 6,
            'source_hashes_verified_before': True, 'selection': plan['selection']}

    def publish():
        write(HERE / 'audit_remote_heterogeneity_status.json', dict(meta, updated_utc=utc(), jobs=state,
    ...
    while pending or active:
        for label, (process, stream, started, job) in list(active.items()):
            ...
            try:
                ...
                assert record['trace_window_ms'] == ([1800., 4200.] if job['trace'] else None)
                if job['trace']:
                    assert sha(record_path.parent / 'trace.json.gz') == record['trace_sha256']
        ...
        # All predecessor queues must have dispatched every job first. Their stale
        # running counts then only overestimate occupied simulation slots.
        while pending and predecessor_pending == 0 and len(active) < 3 and occupied + len(active) < 6:
            job = pending.pop(0)
            label = job['label']
            log = HERE / 'execution_logs' / (label + '.heterogeneity.remote.log')
            stream = log.open('w')
            started = time.perf_counter()
            command = [sys.executable, str(HERE / 'experiment.py'), '--manifest',
            ...
            state[label].update(status='running', pid=process.pid, started_utc=utc(), log=str(log),
                                ...
                                heterogeneity_running_after_start=len(active),
