#!/usr/bin/env python3
"""Import and validate the four transparently constructed remote Baseline inputs."""
...
def verify_one(job, directory, plan, setup, queue):
    ...
    assert metadata['family'] == 'data_affine_extrapolation' and metadata['constructed_profile'] is True
    assert result['metadata']['constructed_profile'] is True
    assert metadata['num_npu'] == 32 and metadata['n_layers'] == 8 and metadata['num_ssu'] == job['num_ssu']
    assert metadata['constructed_builder_sha256'] == job['constructed_builder_sha256'] == plan['frozen_source_sha256']['results/baseline_random_near_capacity_20260914/prepare_constructed.py']
    assert metadata['constructed_plan_sha256'] == job['constructed_plan_sha256'] == plan['frozen_source_sha256']['results/baseline_random_near_capacity_20260914/constructed_candidates.json']
    for row in manifest['requests']:
        load = row['load']
        if load['role'] == 'S':
            assert load['constructed_profile'] is True and load['source_ttft_ms'] is None
            assert load['total_tokens'] == job['short_total_k'] * 1024 and load['nql'] == 128
            assert load['profile_construction']['method'] == 'affine_extrapolation_below_raw_minimum'
        else:
            assert load['role'] == 'L' and load['constructed_profile'] is False
    ...
    return dict(command_sha256=sha(directory / 'command.json'), manifest_sha256=command['manifest_sha256'],
                ...
                all_hashes_blocks_and_invariants_verified=True, construction_provenance_and_final_null_ttft_verified=True)


def sync(password):
    plan = read(HERE / 'audit_remote_constructed_plan.json')
    setup = read(HERE / 'audit_remote_setup.json')
    record_path = HERE / 'audit_remote_constructed_sync_record.json'
...
Q=json.loads((H/'audit_remote_constructed_status.json').read_text());remaining=__REMAINING__
...
if done:
 A=D/('audit_constructed_sync_'+str(time.time_ns())+'.tar.gz')
 with tarfile.open(A,'w:gz') as t:
  for label in done:
   ...
   t.add(H/'execution_logs'/(label+'.constructed.remote.log'),arcname='execution_logs/'+label+'.constructed.remote.log',recursive=False)
...
'''.replace('__DIR__', repr(setup['remote_dir'])).replace('__REMAINING__', repr(remaining))
    ...
    assert queue['plan_sha256'] == sha(HERE / 'audit_remote_constructed_plan.json')
    assert queue['queue_runner_sha256'] == sha(HERE / 'audit_remote_constructed_queue.py')
    assert queue['launch_record_sha256'] == sha(HERE / 'audit_remote_constructed_launch.json')
    launch = read(HERE / 'audit_remote_constructed_launch.json')
    assert launch['construction_audit_passed'] and sha(HERE / launch['audit_report']) == launch['audit_report_sha256']
    (HERE / 'audit_remote_constructed_status.json').write_text(json.dumps(queue, ensure_ascii=False, indent=2) + '\n')
    new = response['new_complete']
    if new:
        incoming = HERE / 'runtime_validation/remote_constructed_incoming'
        ...
        expected.update('execution_logs/' + label + '.constructed.remote.log' for label in new)
        ...
        for label in new:
            ...
            assert not target.exists(), f'Preserving existing constructed result: {target}'
            os.replace(batch / label, target)
            shutil.copyfile(batch / 'execution_logs' / (label + '.constructed.remote.log'),
                            HERE / 'execution_logs' / (label + '.constructed.remote.log'))
    ...
    if finished:
        ...
        acceptance = dict(passed=True, checked_utc=datetime.now(timezone.utc).isoformat(),
                          scope='Four constructed-input Baseline pilots; not direct raw short-request measurements.',
        ...
        (HERE / 'audit_remote_constructed_acceptance.json').write_text(json.dumps(acceptance, ensure_ascii=False, indent=2, allow_nan=False) + '\n')
