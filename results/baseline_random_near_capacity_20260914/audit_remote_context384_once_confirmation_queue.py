...
def main():
    ...
    plan = read(plan_path)
    launch = read(HERE / 'audit_remote_context384_once_confirmation_launch.json')
    assert launch['plan_sha256'] == sha(plan_path) and launch['construction_audit_passed'] is True
    assert plan['max_context384_once_confirmation_parallel'] == 4 and plan['remote_simulation_limit'] == 6
    assert len(plan['jobs']) == 4 and len({job['label'] for job in plan['jobs']}) == 4
    assert all(job['strategy'] == 'once' and job['order'] == 'random' for job in plan['jobs'])
    assert [job['seed'] for job in plan['jobs']] == [19, 43, 67, 101]
    assert all(not job['trace'] for job in plan['jobs'])
    ...
    meta = {'host': platform.node(), 'python_version': sys.version, 'pid': os.getpid(),
            ...
            'launch_record_sha256': sha(HERE / 'audit_remote_context384_once_confirmation_launch.json'),
            'max_context384_once_confirmation_parallel': 4, 'remote_simulation_limit': 6,
            'source_hashes_verified_before': True, 'selection': plan['selection']}

    def publish():
        write(HERE / 'audit_remote_context384_once_confirmation_status.json', dict(meta, updated_utc=utc(), jobs=state,
    ...
    while pending or active:
        ...
        while pending and predecessor_pending == 0 and len(active) < 4 and occupied + len(active) < 6:
            job = pending.pop(0)
            label = job['label']
            log = HERE / 'execution_logs' / (label + '.context384_once_confirmation.remote.log')
            stream = log.open('w')
            ...
            state[label].update(status='running', pid=process.pid, started_utc=utc(), log=str(log),
                                ...
                                context384_once_confirmation_running_after_start=len(active),
