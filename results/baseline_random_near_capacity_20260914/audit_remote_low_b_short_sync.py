#!/usr/bin/env python3
"""Import four low-B-short Baseline cases, including two remotely audited traces."""
...
def verify_one(job, directory, plan, setup, queue):
    ...
    assert len(physical['ssd_busy_ms']) == 4 and len(physical['npu_link_busy_ms']) == 32
    ...
    assert metadata['family'] == 'data_affine_low_b_short' and metadata['constructed_profile'] is True
    assert metadata['num_npu'] == 32 and metadata['n_layers'] == 8 and metadata['num_ssu'] == 4
    ...
    assert metadata['constructed_builder_sha256'] == job['constructed_builder_sha256'] == plan['frozen_source_sha256']['results/baseline_random_near_capacity_20260914/prepare_low_b_short.py']
    assert metadata['constructed_plan_sha256'] == job['constructed_plan_sha256'] == plan['frozen_source_sha256']['results/baseline_random_near_capacity_20260914/low_b_short_math.json']
    for row in manifest['requests']:
        load = row['load']
        if load['role'] == 'S':
            assert load['constructed_profile'] is True and load['source_ttft_ms'] is None
            assert load['total_tokens'] == 10 * 1024 and load['nql'] == 1024
...
def sync(password):
    plan = read(HERE / 'audit_remote_low_b_short_plan.json')
    setup = read(HERE / 'audit_remote_setup.json')
    record_path = HERE / 'audit_remote_low_b_short_sync_record.json'
...
Q=json.loads((H/'audit_remote_low_b_short_status.json').read_text());remaining=__REMAINING__
plan=json.loads((H/'audit_remote_low_b_short_plan.json').read_text());jobs={j['label']:j for j in plan['jobs']}
...
if done:
 ...
 A=D/('audit_low_b_short_sync_'+str(time.time_ns())+'.tar')
 with tarfile.open(A,'w') as t:
  for label in done:
   ...
   t.add(H/'execution_logs'/(label+'.low_b_short.remote.log'),arcname='execution_logs/'+label+'.low_b_short.remote.log',recursive=False)
...
'''.replace('__DIR__', repr(setup['remote_dir'])).replace('__REMAINING__', repr(remaining)).replace('__CHECKER_SHA__', repr(setup['trace_validator']['sha256']))
    ...
    assert queue['plan_sha256'] == sha(HERE / 'audit_remote_low_b_short_plan.json')
    assert queue['queue_runner_sha256'] == sha(HERE / 'audit_remote_low_b_short_queue.py')
    assert queue['launch_record_sha256'] == sha(HERE / 'audit_remote_low_b_short_launch.json')
    launch = read(HERE / 'audit_remote_low_b_short_launch.json')
    assert launch['construction_audit_passed'] and sha(HERE / launch['audit_report']) == launch['audit_report_sha256']
    (HERE / 'audit_remote_low_b_short_status.json').write_text(json.dumps(queue, ensure_ascii=False, indent=2) + '\n')
    new = response['new_complete']
    if new:
        incoming = HERE / 'runtime_validation/remote_low_b_short_incoming'
        ...
        expected.update('execution_logs/' + label + '.low_b_short.remote.log' for label in new)
        ...
        for label in new:
            ...
            assert not target.exists(), f'Preserving existing low-B-short result: {target}'
            os.replace(batch / label, target)
            shutil.copyfile(batch / 'execution_logs' / (label + '.low_b_short.remote.log'),
                            HERE / 'execution_logs' / (label + '.low_b_short.remote.log'))
    ...
    if finished:
        ...
        acceptance = dict(passed=True, checked_utc=datetime.now(timezone.utc).isoformat(),
                          scope='Four low-B-short model stress Baseline pilots; two traces. Extrapolated contexts are not hardware measurements.',
                          ...
                          trace_validation='All retained rows checked on remote host; local saved trace SHA matches that verified file.')
        (HERE / 'audit_remote_low_b_short_acceptance.json').write_text(json.dumps(acceptance, ensure_ascii=False, indent=2, allow_nan=False) + '\n')
