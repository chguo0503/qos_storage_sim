#!/usr/bin/env python3
"""Replay exact archived stripe manifests with ASU or exclusive-NPU OD paths.

No generator is called. Frozen simulator API is used with complete-drain
measurement and independent parity comparison against archived ASU results.
"""
...
def parity_check(result,path):
 ...
 errors=numeric_difference(req(now),req(then),'request_metrics')
 errors+=numeric_difference(layers(now),layers(then),'microbatch_metrics')
 metrics=[]
 for n,o in zip(result['analysis'],old['analysis']):
  metrics.append(dict(window=[n['start_ms'],n['end_ms']],U_error_pp=n['U_percent']-o['U_percent'],same_slo=n['slo']==o['slo'],same_cohort=n['cohort_request_ids']==o['cohort_request_ids']))
  if abs(n['U_percent']-o['U_percent'])>1e-7 or n['slo']!=o['slo'] or n['cohort_request_ids']!=o['cohort_request_ids']:errors.append(dict(path='window_metrics',details=metrics[-1]))
 oldbins=old.get('warm_ssd_10ms_GiB_s')
 if oldbins is None:
  oldbins=[[sum(d[i:i+5])/5 for i in range(0,1000,5)] for d in old['bandwidth_2ms']['ssd_GiB_s']]
 max_bin_error=max(abs(x-y) for a,b in zip(result['warm_ssd_10ms_GiB_s'],oldbins) for x,y in zip(a,b))
 if max_bin_error>1e-6:errors.append(dict(path='warm_ssd_10ms_GiB_s',max_absolute_error=max_bin_error))
 return dict(passed=not errors,reference_sha256=sha(path),same_input=old['input_fingerprint']==result['input_fingerprint'],signature_actual=rows_signature(now),signature_reference=rows_signature(then),numeric_tolerance_ms=1e-7,relative_tolerance=1e-12,errors=errors[:25],error_count=len(errors),window_metrics=metrics,max_SSD_10ms_error_GiB_s=max_bin_error)

def main():
 ap=argparse.ArgumentParser(description=__doc__)
 ap.add_argument('--scenario',choices=('full','semi','near35'),required=True)
 ...
 args=ap.parse_args()
 label=args.label or f'{args.scenario}_{args.policy}_seed{args.seed}'
 ...
 frozen=next(r for r in audit['cases'] if r['scenario']==args.scenario and r['seed']==args.seed)
 assert sha(manifest)==frozen['sha256']
 ...
 record=dict(status='running',scenario=args.scenario,seed=args.seed,policy=args.policy,smoke=args.smoke,argv=sys.argv,pid=os.getpid(),host=platform.node(),python=sys.version,started_utc=utc(),source_sha256=before,manifest_sha256=sha(manifest),input_fingerprint=frozen['input_fingerprint'],expected_blocks=expected,configuration=audit['configuration'],original_input_audit=frozen)
 ...
 try:
  with patch.object(native,'_register_complete',observe):
   result=run_simulation(requests,strategy=args.policy,num_npu=32,num_ssu=3,n_layers=8,seed=args.seed,disk_bw_gib_s=40,npu_bw_gib_s=50,collector_interval_ms=5,cross_request_layer0_prefetch=True)
