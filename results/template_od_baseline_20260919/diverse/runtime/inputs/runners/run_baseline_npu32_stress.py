...
Examples::
  ...
  python -m inputs.runners.run_baseline_npu32_stress --family legacy --num-npu 32 --num-ssu 8 \
      --layout hash --blocks exact --describe-only --manifest-out /tmp/ring_hash.json.gz
  python -m inputs.runners.run_baseline_npu32_stress --manifest /tmp/ring_hash.json.gz \
...
from inputs.manifest import read_json, write_json, save_manifest, load_manifest
from simulator.core.continuous_batch_sim import ContinuousBatchRequest, continuous_batch_input_fingerprint
from inputs.runners.run_multi_ssu_stall_experiments import input_demand
from inputs.runners.run_shared_path_experiments import logical_input_fingerprint, summarize_slo, summarize_window
...
LAYERS = 8
COFLOW_STRATEGIES = ("asu_baseline", "od_baseline", "once", "new_once", "strategy1", "strategy2", "strategy3", "baseline")
...
def profiles_for(family, keys, profile_keys=None):
    ...
    if family in ("legacy", "aligned"):
        from inputs.runners.run_baseline_4npu_ssu1_low_utilization import _profile
...
def build_workload(*, family="legacy", num_npu=32, num_ssu=8, layout="hash",
                   blocks="exact", raw_keys="32:256,192:1024,192:2048,192:2048",
                   profile_keys=None,
                   horizon_ms=4000.0, compute_scale=1.0, target_rho=None,
                   seed=20260908, order="fixed"):
    if layout != "hash":
        raise ValueError("Only ring-hash placement is supported; use layout='hash'")
    ...
    def layer_for(profile, rid):
        ...
        return tuple((sim.block_ring_hash_disk_id(rid, j, num_ssu), size)
    ...
    metadata = {"experiment": "baseline_npu32_stress_v1", "family": family,
        ...
        "regime": "saturated_finite_backlog", "layout": sim.PLACEMENT_BLOCK_RING_HASH, "blocks": blocks,
        "placement_identity_key": "request_id",
        "placement_rule": "block_ring_hash_disk_id(request_id, block_index, num_ssu); same physical block across layers",
        ...
        "equal_176kib_blocks": equal_blocks,
...
def run_case(requests, metadata, *, strategy="baseline_native", assignment="pipeline",
    ...
    if strategy in COFLOW_STRATEGIES:
        ...
        from inputs.runners.run_coflow_experiments import run_case as run_coflow
        ...
        from inputs.runners.run_multi_ssu_stall_experiments import run_case as run_native
...
def main():
    ...
    parser.add_argument("--layout", choices=("hash",), default="hash",
                        help="Only ring hash is supported for newly generated inputs")
    ...
    if args.manifest:
        ...
        requests, metadata = build_workload(**{key: getattr(args, key) for key in (
            ...
            "compute_scale", "target_rho", "seed", "order")})
