...
def main():
    ...
    assert reorder_pending(commands, .5) == (11, 10)
    assert commands[0].io_id == 10
    try:
        reorder_pending(commands, 0)
    except ValueError:
        pass
    else:
        raise AssertionError("future commands must be rejected")
    print("strategy2: PASS (pending-only deadline order; future commands rejected)")
