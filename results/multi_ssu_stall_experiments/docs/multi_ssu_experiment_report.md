...
本轮所有已保存性能实验共 44 组；固定窗口内每张卡始终有已接纳工作：全部通过。本轮最高窗口值为 99.732%（6盘，20260907，局部stall交换），但它不构成最优证明，也不等于所有种子、P99 和整批完成时间都最好。**近容量原始 data 变化输入没有自动重现旧构造中“baseline 很低、新策略几乎 100%”的大幅差距。**
...
| 长度K | NQL | 块/层 | MiB/层 | C/ms | 6盘配额 | 7盘配额 |
|---|---|---|---|---|---|---|
...
| SSU | 种子 | 策略 | 完工ms | 均延迟ms | P99 ms | 整批U% |
|---|---|---|---|---|---|---|
...
| 6 | 06 | B: A+fluid选卡 | 4930.7 | 889.3 | 1365.4 | 89.35 |
...
| 6 | 07 | B: A+fluid选卡 | 4836.4 | 849.4 | 1318.7 | 91.09 |
...
| 7 | 06 | B: A+fluid选卡 | 4234.1 | 758.8 | 1223.8 | 91.22 |
...
| 7 | 07 | B: A+fluid选卡 | 4222.7 | 705.4 | 1136.3 | 91.47 |
...
| SSU | 种子 | Baseline% | Once% | A% | B% | 全卡active |
|---|---|---|---|---|---|---|
...
特别看6盘：B把主窗口利用率从91.891%提高到98.394%，但完整请求集的完工时间仅从4658.8ms变为4643.5ms，缩短0.33%；仍慢于A的4458.4ms。不能把这一秒的约6.5个百分点增益当作同幅度的长期吞吐提升。

## 种子06的近容量消融及新候选复核

| SSU | 策略 | 窗口U% | 完工ms | P99 ms |
|---|---|---|---|---|
| 6 | Baseline | 93.260 | 4808.4 | 2000.8 |
| 6 | A+compute选卡 | 96.379 | 4932.3 | 1361.1 |
| 6 | B: A+fluid选卡 | 95.216 | 4930.7 | 1365.4 |
| 6 | A: deadline | 95.731 | 4643.9 | 1875.9 |
| 6 | A: deadline / 0.1ms限频 | 95.770 | 4641.9 | 1871.8 |
| 6 | deadline预留 | 95.698 | 4634.8 | 1882.2 |
| 6 | V/C max-min | 94.131 | 4769.6 | 1983.6 |
| 6 | Once/layer | 94.832 | 4731.6 | 1970.5 |
| 6 | 局部stall交换 | 96.068 | 4616.9 | 1891.3 |
| 7 | Baseline | 97.360 | 4111.6 | 1679.3 |
| 7 | A+compute选卡 | 98.615 | 4293.8 | 1242.2 |
| 7 | B: A+fluid选卡 | 99.046 | 4234.1 | 1223.8 |
| 7 | A: deadline | 99.678 | 3946.5 | 1669.4 |
| 7 | A: deadline / 0.1ms限频 | 99.676 | 3948.2 | 1668.8 |
| 7 | deadline预留 | 99.633 | 3945.2 | 1669.5 |
| 7 | V/C max-min | 97.979 | 4035.6 | 1688.9 |
| 7 | Once/layer | 98.637 | 4032.9 | 1687.5 |
| 7 | 局部stall交换 | 99.673 | 3954.0 | 1667.4 |

局部stall交换按两个种子单独对照A；下面时间差都是“新候选减A”，完工或P99的正值表示变差。它不是每个指标都优于A，也不能因单瓶颈代理目标下降就宣布真实多盘目标单调下降。

| SSU | 种子 | A% | 交换% | 差/百分点 | 完工差ms | P99差ms |
|---|---|---|---|---|---|---|
...
- **有完整冷启动输入的条件预测**：本轮 10 个32卡6/7盘case、共 3088 层，对原生到齐、计算开始、未来stall的最大绝对误差 0 ms。包含原 data 多种请求和单卡接收瓶颈回归。相同初始输入/原生公开种子允许重现同刻提交顺序，不是读取结果后重放。
...
| SSU | 快照ms | 正在计算 | 到齐误差 | stall误差 | 漏报 | 误报 |
|---|---|---|---|---|---|---|
...
|---|---|---|---|---|
| 6 | A+compute选卡 | 704 | 1368.9 | 0.964 |
| 6 | B: A+fluid选卡 | 704 | 1130.6 | 0.796 |
| 7 | A+compute选卡 | 704 | 1432.5 | 1.008 |
| 7 | B: A+fluid选卡 | 704 | 1916.1 | 1.349 |
