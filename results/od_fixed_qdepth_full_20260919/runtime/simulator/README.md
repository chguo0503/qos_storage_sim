...
可通过 API 开启每盘固定的 I/O 名额；`None` 保留原来不限制队列深度的 OD：

```python
result = run_simulation(
    requests, strategy="od_baseline", num_npu=32, num_ssu=3,
    n_layers=8, seed=7, od_queue_depth_per_ssu=8192,
)
```

这里的 8192 是**每盘的 I/O 条数**，不是字节数。总深度必须能被 NPU 数整除；32 卡各固定分到 256 条，空闲名额不能借给其他卡。占用从实际下发开始，覆盖盘内排队和 SSD 服务；SSD 完成后立即归还，不等待后续 NPU 接收链路完成。256 条 176 KiB I/O 相当于每卡每盘最多 44 MiB 的盘内在途数据。

某卡向某盘的名额用满后，其未发出的 I/O 保留在主机侧；其他盘仍可继续下发。名额释放后补发，保留原来每卡每 0.1 微秒发一条的节拍，当前不额外模拟名额返回的通信延迟。层读取起点和完成屏障覆盖这些未下发 I/O，因此主机侧等待不会从层时延或 TTFT 中消失。

该参数只限制深度，OD 的 CIR、PIR 和空闲带宽借用规则保持原样。深度固定不借用与带宽允许借用是两件事。盘内排队缩短也不一定代表请求提速，需同时查看主机等待、完整层时延和请求完成时间。
