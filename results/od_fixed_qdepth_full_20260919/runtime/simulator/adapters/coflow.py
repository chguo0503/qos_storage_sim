...
from simulator.policies.coflow_client_policy import ClientRead, choose_npu, choose_npu_pipeline, plan_global_batch
...
class GlobalClient:
    ...
    def global_priority(self, flow, now_ms):
        from simulator.policies.coflow_joint_policy import coflow_priority
...
def coflow_adapter(policy="strategy1", collector_interval_ms=5.0,
    ...
    with shared_path_adapter("new_once", collector_interval_ms, cir_min_interval_ms) as shared:
        ...
        def arrival(context, rid, now_ms):
            ...
            scores = tuple(n.remaining_compute_ms for n in backlogs)
            if assignment == "pipeline":
                assigned, scores = choose_npu_pipeline(backlogs, shared.layer_counts(request)[0],
                    request.per_layer_compute_ms, original.npu_id, n_layers=context.n_layers,
            ...
            client.assignment_log.append({"request_id": rid, "arrival_time_ms": now_ms,
                ...
                "scores_ms": scores, "score_model": assignment,
        ...
        if policy == "strategy1":
            ...
            from simulator.adapters.coflow_disk import coflow_disk_adapter
