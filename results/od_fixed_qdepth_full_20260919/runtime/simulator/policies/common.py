...
def main():
    ...
    counts = (3,) + (0,) * 31 + (2,) + (0,) * 223
    view = pressure_from_counts(counts, config)
    assert sum(view.group_io_counts) == 5
    assert view.active_paths_per_group[:2] == (1, 1)
    assert view.active_group_weight_sum == 2
    assert solo_io_ms((0, 0)) == 0
    assert solo_io_ms((100, 0)) >= 1000 * IO_GIB * 100 / 40
    print("common: PASS (count aggregation and isolated service lower bound)")
