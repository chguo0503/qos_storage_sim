...
def main():
    ...
    witnesses = capacity_overloads(reads, 0)
    assert witnesses and any(w.resource == "ssu" for w in witnesses)
    assert capacity_overloads((), 0) == ()
    assert solo_read_lower_bound_ms((0, 0)) == 0
    print("coflow_capacity_analysis: PASS (overload witness and empty workload)")
