...
from simulator.config import routing_strategy_specs, static_qos_config
from simulator.policies.baselines import baseline_configuration, baseline_qos_config, canonical_baseline_name
from inputs.runners.run_shared_path_experiments import (
    ...
    source_files, summarize_slo, summarize_window,
)


OUT = ROOT / "results/coflow_global_5ms_experiments/data"
STRATEGIES = ("asu_baseline", "od_baseline", "once", "new_once", "strategy1", "strategy2", "strategy3", "baseline")
REFERENCE_STRATEGIES = ("asu_baseline", "od_baseline", "baseline", "once", "new_once")
...
def retime_arrivals(requests, target_gib_s=TARGET_ARRIVAL_GIB_S):
    """Preserve canonical reference arrival/rid order and the original tuple.
    ...
    is never divided by an artificial observation interval. Reference arrival
    times are rounded to 1 ps before sorting and storing: Python versions can
    otherwise disagree by a few float ulps on theoretically tied sums, then
    amplify that tie reversal into a full-request arrival-clock difference.
    Quantization does not promise invariance to perturbations across a bin's
    half-way boundary; it fixes the measured cross-version differences here.
    """
    canonical_reference_ms = {r.request_id: round(r.arrival_time_ms, 9) or 0.0
                              for r in requests}
    postinitial = sorted((r for r in requests if canonical_reference_ms[r.request_id] > 0),
                         key=lambda r: (canonical_reference_ms[r.request_id], r.request_id))
    cumulative_bytes = 0
    arrival_by_id = {}
    ...
    for request in requests:
        ...
        load.update(reference_arrival_ms=canonical_reference_ms[request.request_id],
...
def arrival_workload(requests, num_ssu):
    ...
    return {
        ...
        "reference_arrival_quantum_ms": 1e-9,
        "retiming_rule": "canonical_reference_ms = round(reference_ms, 9), normalize zero; sort positive canonical arrivals by (canonical_reference_ms, request_id); arrival_ms = 1000*cumulative_8layer_bytes/(2**30*target_gib_s)",
        "reference_quantization_caveat": "1 ps quantization removes observed cross-Python summation tie drift; it does not guarantee perturbation invariance across arbitrary rounding-bin boundaries",
...
def run_case(requests, metadata, *, strategy="asu_baseline", collector_interval_ms=5.0,
    ...
    if strategy in REFERENCE_STRATEGIES:
        ...
        from simulator.adapters.coflow import coflow_adapter
        manager = coflow_adapter(policy=strategy, collector_interval_ms=collector_interval_ms,
                                 cir_min_interval_ms=cir_min_interval_ms,
                                 queue_window_ms=queue_window_ms, assignment=assignment,
    ...
    canonical_strategy = canonical_baseline_name(strategy)
    route = "baseline" if canonical_strategy in ("asu_baseline", "od_baseline") else "layer_once"
    client = next(s for s in routing_strategy_specs() if s.name == route).client_config()
    qos = baseline_qos_config(strategy, metadata["num_npu"], DISK_GIB_S)
    ...
    return {
        ...
        "baseline_configuration": baseline_configuration(strategy, metadata["num_npu"], DISK_GIB_S),
        ...
        "policy_config": {"queue_window_ms": queue_window_ms,
                          "assignment": assignment, "joint_rule": joint_rule},
        "cir_policy": ("equal static CIR per exclusive NPU path; idle capacity may be borrowed"
                       if canonical_strategy == "od_baseline" else
                       "shared FINAL_STATIC; no runtime CIR changes"),
...
def main():
    ...
    try:
        result = run_case(requests, metadata, strategy=args.strategy,
                          queue_window_ms=args.queue_window_ms, assignment=args.assignment,
