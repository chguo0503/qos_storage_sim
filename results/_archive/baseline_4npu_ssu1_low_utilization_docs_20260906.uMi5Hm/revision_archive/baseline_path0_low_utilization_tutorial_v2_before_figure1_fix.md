...
author: "qos_storage_sim 可复现实验 · v2.1（图 4 可读性修订）"
date: "2026-09-05"
...
图 4 的 A 图为精确计算/等待事件；B 图把 SSD 服务按 0.1 ms 汇总成时间占比；C 图放大到 8.000–8.100 ms，显示命令真实交替顺序。A/B 的横轴单位为 ms，C 为 µs。

![同一 NPU 在三个子图中颜色一致。A 图的 NPU2 绿条、NPU3 紫条表示持续计算；B 图的分层色带是分桶后的时间比例，不是 SSD 同时并行服务；C 图给出精确的串行命令顺序。](05_phase_lock_zoom_readable.pdf){width=100%}
