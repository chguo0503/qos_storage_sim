...
def plot_npu_timeline_readable(result: dict, layer_rows: list[dict]) -> None:
    ...
    vector = RESULT / "01_npu_io_compute_timeline_readable.pdf"
    ...
    _save(fig, RESULT / "01_npu_io_compute_timeline_readable.png")
...
def plot_zoom(result: dict, layer_rows: list[dict], block_rows: list[dict]) -> None:
    ...
    _save(fig, RESULT / "05_phase_lock_zoom.png")
...
def plot_phase_columns(result: dict, layer_rows: list[dict]) -> None:
    ...
    _save(fig, RESULT / "06_release_phase_columns.png")
...
def plot_zoom_readable(result: dict, layer_rows: list[dict], block_rows: list[dict]) -> None:
    """Figure 4: one identity palette, explicit wait state, truthful multiscale view.

    Legacy plot_zoom remains available for the old PDF. The new figure uses
    ...
    grid = fig.add_gridspec(
        3, 1, height_ratios=(3.5, 1.8, 1.0), hspace=0.65,
        left=0.15, right=0.985, bottom=0.085, top=0.855)
    ax = fig.add_subplot(grid[0])
    service_ax = fig.add_subplot(grid[1], sharex=ax)
    micro_ax = fig.add_subplot(grid[2])
    fig.suptitle("图 4｜同一 NPU，始终使用同一种颜色", fontsize=16, y=0.993)
    fig.legend(
        handles=[Patch(facecolor=COLORS[n], label=f"NPU{n}") for n in range(4)],
        loc="upper center", bbox_to_anchor=(0.565, 0.96), ncol=4,
        fontsize=12, frameon=False, handlelength=2.0, columnspacing=1.9)
    ...
    if len(pacer_releases) >= 2:
        ...
        ax.text((x0 + x1) / 2, 3.76, f"周期 {period:.4f} ms", ha="center", fontsize=10.5)
    ...
    fig.text(0.15, 0.015, "A、C 保留精确事件；B 仅为可读性做时间加权显示。原始仿真数据未改。", fontsize=10.5)

    vector = RESULT / "05_phase_lock_zoom_readable.pdf"
    temporary = vector.with_name(vector.name + ".tmp")
    with matplotlib.rc_context({"pdf.fonttype": 42}):
        fig.savefig(temporary, format="pdf", bbox_inches="tight", facecolor="white")
    temporary.replace(vector)
    _save(fig, RESULT / "05_phase_lock_zoom_readable.png")
...
def plot_search(search_rows: list[dict[str, str]]) -> None:
    ...
    _save(fig, RESULT / "07_bounded_search_curve.png")
...
def plot_feedback_loop() -> None:
    ...
    fig.tight_layout()
    _save(fig, RESULT / "08_closed_loop_feedback.png")
...
def plot_queue_causality(audit: dict) -> None:
    ...
    fig.tight_layout()
    _save(fig, RESULT / "09_npu1_queue_causality.png")


def main() -> None:
    _configure_matplotlib()
    with (RESULT / "result.json").open(encoding="utf-8") as handle:
        result = json.load(handle)
    layer_rows = _read_rows(RESULT / "request_layer_timeline.csv")
    block_rows = _read_rows(RESULT / "physical_block_trace.csv")
    search_rows = _read_rows(RESULT / "search_summary.csv")
    plot_npu_timeline_readable(result, layer_rows)
    plot_zoom_readable(result, layer_rows, block_rows)
    plot_phase_columns(result, layer_rows)
    plot_search(search_rows)
    plot_feedback_loop()
    audit_path = RESULT / "tutorial_numeric_audit.json"
    if audit_path.exists():
        with audit_path.open(encoding="utf-8") as handle:
            plot_queue_causality(json.load(handle))
    print("generated readable figures 01 and 05 (PDF/PNG), figures 06--08, and 09 when the numeric audit is available")
