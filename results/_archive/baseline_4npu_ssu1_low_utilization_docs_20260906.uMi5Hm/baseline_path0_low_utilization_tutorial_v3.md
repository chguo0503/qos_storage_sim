...
author: "qos_storage_sim 可复现实验 · v3 新手讲解版"
date: "2026-09-06"
...
**时间词典。** “7.562 ms 发起”是一个**时刻**；“读到齐用了 12.859 ms”是一段**时长**，不能在横轴上直接寻找“12.859”这个完成点。完成点是“发起时刻 + 延迟”。图中文字会四舍五入，最后一位可能有舍入差；绘制位置与核算使用原始精度。
...
![完整一秒总览（A）与 NPU1 分层放大（B）。蓝色为层读取未完成区间，包含排队、服务与接收；青色为计算，红色为等 I/O。图中 b–c 的读取空白期间，NPU1 仍在计算 L3。](01_npu_io_compute_timeline_readable.pdf){width=100%}
...
**阅读顺序：先看 A 的计算与发起，再看 A 底部读取标注；然后看 B 的服务占比。不要把 A 的计算条当作 B 的读取条。** P 的推导放在本节后面的“周期账本”图中，暂时只把长虚线当作观察重复的参考标记。

图 4 的 A 图为精确计算/等待事件，并在底部另加 NPU2 的 L2 读取标注；B 图把 SSD 服务按 0.1 ms 汇总成时间占比；C 图放大到 8.000–8.100 ms，显示命令真实交替顺序。A/B 的横轴单位为 ms，C 为 µs。

![A 的彩色实心条表示计算，短黑竖线表示发起读取；底部另标 NPU2/L2 从发起到 HBM 到齐的 12.859 ms。B 是实际 SSD 服务的时间占比，C 是精确命令顺序。长虚线仅标参考周期，不是读取完成时刻。](05_phase_lock_zoom_annotated.pdf){width=100%}

**先分清三个时间。** 对 NPU2 的请求 200010、L2：7.561568 ms 发起读取，20.420636 ms 到达 HBM，延迟为 **12.859069 ms**；37.418131 ms 才开始计算 L2 并发起 L3 读取。因此两根发起标记间的 **29.856563 ms** 是计算/释放周期，不是读一层用了这么久。L2 到齐之后，NPU2 仍计算 L1 约 16.997494 ms，没有等待 I/O。

这次 L2 的物理 SSD 服务分布在 7.733611–20.417279 ms，横向跨度为 12.683669 ms；其中 SSD **累计真正服务 NPU2 的时间只有 6.415939 ms**，其余时间服务其它卡。服务累计时间、服务分布跨度、发起到 HBM 到齐的延迟，不能混为同一个“读取时间”。图中数字四舍五入显示，绘制位置保留 trace 精度。
...
![图 4A 的四卡 0–10 ms 放大，以及同一时段的 SSD 时间占比。条内 Lx 是计算层号，黑竖线旁“读Lx”是发起的读取层号；两者通常相差一层。下图黑框选中 5.2–5.3 ms 的计算示例。](10_npu_0_10ms_zoom.pdf){width=100%}

NPU0 在约 7.589 ms 发起 L6 读取，8.171 ms 计算完 L5 后开始等 L6，直到 10.131 ms 才读齐；这个结束时刻略超出图的右边界。NPU2 在 7.562 ms 发起 L2，20.421 ms 才读齐，其终点也不在 0–10 ms 视窗内，须回看图 4 底部的完整标注。**右边界 10 ms 不表示请求完成或读取结束。**
...
在图 4 A 顶部，NPU3 相邻两次发起标记是 7.557371713 ms 和 37.413934529 ms。把后一时刻减去前一时刻：
...
![周期账本。先由 NPU3 两个发起时刻确定 P，再累计同一时间段中各卡的计算与等待。色块是累计时长的分解，不表示把所有计算真实排在等待之前。](11_cycle_budget_explained.pdf){width=100%}
...
![NPU1 的完整因果例子。主图 a–e 与同图事件表一一对应；d/e 仅差约 3.252 微秒，另作尾部放大。下方预算条表示前一层 L2 的计算，不是 L3 已经在计算。](09_npu1_queue_causality_annotated.pdf){width=100%}
...
![NPU0 同一请求的两层读取。a–e 的精确时刻与事件表对齐：前一次等待结束，会同时触发下一次读取，但新读取仍可能再次遇到长队列。图只画这两个选定的读取任务。](12_npu0_two_layer_story.pdf){width=100%}
...
![两个函数预测例子，不是实测时间线。横轴均以预测当下为 0，并直接标出预算截止和预计完成时刻。例 A 被 IOPS 上限约束；例 B 使用 NPU0 的 V/C，但假定前方已有 256 MiB。](13_fifo_budget_examples.pdf){width=100%}
...
![NPU0 深预取的时间与 HBM 预算。直接标出 2C、29C、30C 的累计时长；竖线是用于估算的 16.7 ms 坏排队参照，不是新的实测事件。](14_prefetch_budget.pdf){width=100%}
...
原始结果、输入表、搜索表和 trace 未修改。当前新手版另存为带 `_v3` 的 Markdown/PDF；旧版 PDF 保留，之前的源码还在 `revision_archive/` 留有备份。
...
python -m unittest test_path_deadline_estimator test_baseline_path0_tutorial_assets test_baseline_path0_beginner_assets
