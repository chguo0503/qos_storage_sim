...
from simulator.policies.allocation import (
    GrantMatrix,
...
def main():
    rows = (ManifestDemand(0, 0, .01, (.6, .1)),
            ManifestDemand(1, 1, .01, (.6, .1)))
    plan = plan_scheme_b(rows, num_npu=2, num_ssu=2, path_by_npu=(0, 32))
    assert validate_scheme_b_plan(plan)
    assert plan.grants_gbps == ((20.0, 10.0), (20.0, 10.0))
    assert plan.path_cirs_by_ssu[0][0] == plan.path_cirs_by_ssu[0][32] == 20
    assert baseline_path_ids(3) == (0, 0, 0)
    assert dedicated_path_id(0) != dedicated_path_id(1)
    print("policy_logic: PASS (portable routing helpers and feasible Scheme-B grants)")
