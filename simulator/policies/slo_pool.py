...
def main():
    """Validate fallback, balanced candidate pools, and Once delegation."""
    ...
    allowed = category_path_ids("SS", qos)
    snapshot = pressure_from_counts((0,) * 256, qos)
    unknown = choose_path_pool(allowed, qos, budget=None)
    assert unknown.path_ids == allowed and unknown.mode == "unknown_budget_once"
    urgent = RequestBudget(12.0, 8.0, 8, (0.001, 0.001, 0.001))
    slack = RequestBudget(300.0, 200.0, 8, (0.001, 0.001, 0.001))
    assert choose_path_pool(allowed, qos, budget=urgent).path_ids == allowed
    selected = choose_path_pool(allowed, qos, budget=slack)
    assert len(selected.path_ids) == 8
    assert {p // qos.paths_per_group for p in selected.path_ids} == set(range(8))
    expected = once_path_ids(16, snapshot, selected.path_ids, qos)
    assert slo_path_ids(16, snapshot, allowed, qos, budget=slack) == expected
    assert slo_path_ids(16, snapshot, allowed, qos, budget=None) == once_path_ids(16, snapshot, allowed, qos)
    print("slo_pool: PASS (fallback, 8 balanced paths, unchanged Once routing)")
