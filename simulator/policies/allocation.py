...
def main():
    demand = ((60.0, 10.0), (60.0, 10.0))
    grants = allocate_grants(demand)
    assert grants == ((20.0, 10.0), (20.0, 10.0))
    coflow = allocate_coflow_grants(demand, target_ratios=0.5)
    for s in range(2):
        assert sum(row[s] for row in coflow) <= 40 + 1e-9
    for wanted, got in zip(demand, coflow):
        assert sum(got) <= 50 + 1e-9
        assert all(0 <= x <= y + 1e-9 for x, y in zip(got, wanted))
        assert math.isclose(got[0] / wanted[0], got[1] / wanted[1])
    print("allocation: PASS (demand caps, disk/link caps and coflow progress ratios)")
