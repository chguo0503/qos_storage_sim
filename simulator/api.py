...
from simulator.policies.od_baseline import queue_depth_per_npu
...
def run_simulation(requests, *, strategy="asu_baseline", num_npu=32, num_ssu=3,
    ...
    Optional OD queue depth counts queued and active SSD commands; each NPU
    has a fixed equal quota on each SSD. SSD completion releases a slot before
    NPU-link transfer. ``None`` preserves the historical unbounded client.
    """
    strategy = canonical_baseline_name(strategy)
    if strategy not in STRATEGIES:
        raise ValueError(f"strategy must be one of {STRATEGIES}")
    if od_queue_depth_per_ssu is not None and strategy != "od_baseline":
        raise ValueError("od_queue_depth_per_ssu is only supported by od_baseline")
    depth = (queue_depth_per_npu(od_queue_depth_per_ssu, num_npu)
             if od_queue_depth_per_ssu is not None else None)
