...
from simulator.policies.policy_logic import baseline_path_ids as policy_baseline_path_ids
# Re-export established public names while keeping policy decisions out of the engine.
from simulator.contracts import (
    ControlRequestView, CIRControlSnapshot, CIRControlDecision, CIRControlCallback,
    CIRControlConfig, CausalLayerObservation, CausalLayerSnapshot,
    CausalLayerControlCallback, CausalLayerControlConfig,
)
from simulator.policies.scheme_b import MaxMinSchemeBController
from simulator.policies.scheme_b_slo import SLOAwareSchemeBController
from simulator.policies.scheme_b_causal import CausalMaxMinSchemeBController
...
class ContinuousBatchRequest:
    ...
    ``placement[layer][block]`` is ``(ssu_id, size_gb)`` and remains immutable.
    Request IDs must be globally unique. Builders that renumber requests after
    queue shuffling hash ``load['original_request_id']`` instead, so changing
    admission order or NPU assignment does not relocate existing blocks.
    """
...
class _SubmissionState:
    ...
    depth_blocked_since_ms: Optional[float] = None
...
class _Context:
    def __init__(
        ...
        *,
        ...
        oracle_priority_key,
        ssd_queue_depth_per_npu: Optional[int] = None,
    ):
        ...
        # Optional fixed command slots, independent of bandwidth borrowing.
        # Entries remain occupied through SSD service but not NPU-link service.
        self.ssd_queue_depth_per_npu = ssd_queue_depth_per_npu
        self.ssd_depth_outstanding = [[0] * num_ssu for _ in range(num_npu)]
        self.ssd_depth_peak = [[0] * num_ssu for _ in range(num_npu)]
        self.ssd_depth_blocked_episodes = [[0] * num_ssu for _ in range(num_npu)]
        self.ssd_depth_host_wait_ms = [[0.0] * num_ssu for _ in range(num_npu)]
        self.ssd_depth_waiters: set[tuple[int, int]] = set()
        self.ssd_depth_activated_blocks = 0
        self.ssd_depth_issued_blocks = 0
        self.ssd_depth_completed_blocks = 0
        self.ssd_depth_host_enqueue_wait_ms = 0.0
        self.ssd_depth_completed_host_wait_ms = 0.0
        self.ssd_depth_completed_enqueued_latency_ms = 0.0
        self.ssd_depth_completed_activated_latency_ms = 0.0
...
def _register_submit(context: _Context, flow: sim.BlockIOFlow):
    ...
    if context.ssd_queue_depth_per_npu is not None:
        n, s = flow.npu_id, flow.disk_id
        context.ssd_depth_outstanding[n][s] += flow.block_count
        context.ssd_depth_issued_blocks += flow.block_count
        outstanding = context.ssd_depth_outstanding[n][s]
        if outstanding > context.ssd_queue_depth_per_npu:
            raise AssertionError("SSD submission exceeded the fixed NPU queue quota")
        context.ssd_depth_peak[n][s] = max(context.ssd_depth_peak[n][s], outstanding)
        context.ssd_depth_host_enqueue_wait_ms += max(
            0., flow.enqueue_time-request_state.io_start_time_ms[flow.layer])
...
def _register_complete(context: _Context, flow: sim.BlockIOFlow):
    ...
    if context.ssd_queue_depth_per_npu is not None:
        activated = context.requests[flow.request_id].io_start_time_ms[flow.layer]
        context.ssd_depth_completed_host_wait_ms += max(0., flow.enqueue_time-activated)
        context.ssd_depth_completed_enqueued_latency_ms += context.current_time_ms-flow.enqueue_time
        context.ssd_depth_completed_activated_latency_ms += context.current_time_ms-activated
...
def _depth_can_submit(context: _Context, state: _SubmissionState):
    """Check one SSD independently; record full-quota host wait intervals."""
    n, s = state.npu_id, state.disk_id
    available = context.ssd_depth_outstanding[n][s] < context.ssd_queue_depth_per_npu
    if not available and state.ready_time_ms <= context.current_time_ms + _EPS:
        context.ssd_depth_waiters.add((n, s))
        if state.depth_blocked_since_ms is None:
            state.depth_blocked_since_ms = context.current_time_ms
            context.ssd_depth_blocked_episodes[n][s] += 1
    return available


def _release_ssd_queue_slot(context: _Context, flow: sim.BlockIOFlow, now_ms):
    """Release on physical SSD completion, with zero artificial feedback delay."""
    n, s = flow.npu_id, flow.disk_id
    context.ssd_depth_outstanding[n][s] -= flow.block_count
    context.ssd_depth_completed_blocks += flow.block_count
    if context.ssd_depth_outstanding[n][s] < 0:
        raise AssertionError("SSD queue slot released without a submitted command")
    if (n, s) not in context.ssd_depth_waiters:
        return False
    context.ssd_depth_waiters.remove((n, s))
    for state_id in context.submission_queues.get(n, ()):
        state = context.submission_states[state_id]
        if state.disk_id == s and state.depth_blocked_since_ms is not None:
            context.ssd_depth_host_wait_ms[n][s] += max(0., now_ms-state.depth_blocked_since_ms)
            state.depth_blocked_since_ms = None
    return True
...
def _schedule_client_event(context: _Context):
    if not context.submission_states:
        return
    if context.ssd_queue_depth_per_npu is None:
        next_times = [
            max(
        ...
        # A full SSD must not hide an eligible state for another SSD behind it.
        # No polling is scheduled when every state is blocked; SSD completion
        # wakes the client after freeing a slot.
        next_times = [
            max(context.current_time_ms, state.ready_time_ms,
                context.client_next_issue_ms[npu_id])
            for npu_id, state_ids in context.submission_queues.items()
            for state_id in state_ids
            for state in (context.submission_states[state_id],)
            if _depth_can_submit(context, state)
        ]
    if not next_times:
        return
...
def _start_layer_io(
    ...
    *,
    ...
    for ssu_id, blocks, work_gb in placement_groups:
        ...
        if context.ssd_queue_depth_per_npu is not None:
            context.ssd_depth_activated_blocks += len(blocks)
...
def _submit_one_client_batch(
    ...
    if context.ssd_queue_depth_per_npu is not None:
        free_slots = context.ssd_queue_depth_per_npu - context.ssd_depth_outstanding[state.npu_id][state.disk_id]
        submit_end = min(submit_end, state.cursor + free_slots)
...
def _handle_client_submission(context: _Context, generation, current_time_ms):
    ...
    for npu_id, state_ids in context.submission_queues.items():
        ...
        if context.ssd_queue_depth_per_npu is None:
            ...
            state = next((context.submission_states[sid] for sid in state_ids
                          if _depth_can_submit(context, context.submission_states[sid])
                          and context.submission_states[sid].ready_time_ms <= current_time_ms + _EPS), None)
            if state is None:
                continue
        if (
            ...
            selected_states[npu_id] = state.state_id
    ready_npus.sort()
    ...
    for npu_id in ready_npus:
        ...
        if context.ssd_queue_depth_per_npu is None:
            ...
            state_id = selected_states[npu_id]
    ...
    for npu_id, state_ids, state_id, state in issue_round:
        ...
        issued = state.cursor - before_cursor
...
def _handle_disk_completion(
    ...
    for flow in completed:
        if context.ssd_queue_depth_per_npu is not None:
            wake_client |= _release_ssd_queue_slot(context, flow, current_time_ms)
...
def _add_ssd_queue_depth_summary(context, summary, current_time_ms):
    """Opt-in diagnostics only; preserve default summary and event semantics."""
    depth = context.ssd_queue_depth_per_npu
    if depth is None:
        return summary
    deferred = [[0] * context.num_ssu for _ in range(context.num_npu)]
    host_wait = [list(row) for row in context.ssd_depth_host_wait_ms]
    for state in context.submission_states.values():
        deferred[state.npu_id][state.disk_id] += len(state.blocks)-state.cursor
        if state.depth_blocked_since_ms is not None:
            host_wait[state.npu_id][state.disk_id] += max(0., current_time_ms-state.depth_blocked_since_ms)
    outstanding = sum(map(sum, context.ssd_depth_outstanding))
    host_pending = sum(map(sum, deferred))
    link_pending = context.ssd_depth_completed_blocks-context.completed_blocks
    checks = {
        "ssd_depth_never_exceeded": all(0 <= v <= depth for row in context.ssd_depth_peak for v in row),
        "ssd_depth_host_block_conservation": context.ssd_depth_activated_blocks == context.ssd_depth_issued_blocks+host_pending,
        "ssd_depth_device_block_conservation": context.ssd_depth_issued_blocks == context.ssd_depth_completed_blocks+outstanding,
        "ssd_depth_link_block_conservation": link_pending >= 0,
        "ssd_depth_latency_decomposition": math.isclose(
            context.ssd_depth_completed_host_wait_ms+context.ssd_depth_completed_enqueued_latency_ms,
            context.ssd_depth_completed_activated_latency_ms, rel_tol=1e-10, abs_tol=1e-8),
    }
    if context.completed_requests == len(context.requests):
        checks["ssd_depth_finite_input_drained"] = (
            host_pending == outstanding == link_pending == 0
            and context.ssd_depth_activated_blocks == context.expected_blocks)
    summary["invariants"].update(checks)
    summary["ssd_queue_depth"] = {
        "enabled": True,
        "per_npu_per_ssu_slots": depth,
        "total_reserved_slots_per_ssu": depth*context.num_npu,
        "slot_scope": "one command queued at or currently serviced by SSD; excludes NPU link",
        "slot_release": "SSD completion, zero additional modeled feedback delay",
        "queue_slot_borrowing": False,
        "bandwidth_policy_changed": False,
        "matrix_order": "[npu_id][ssu_id]",
        "peak_outstanding_blocks_by_npu_ssu": context.ssd_depth_peak,
        "end_outstanding_blocks_by_npu_ssu": context.ssd_depth_outstanding,
        "end_host_deferred_blocks_by_npu_ssu": deferred,
        "blocked_state_episodes_by_npu_ssu": context.ssd_depth_blocked_episodes,
        "host_blocked_state_ms_by_npu_ssu": host_wait,
        "host_blocked_time_definition": "sum over submission-state ready intervals with full quota; overlapping states add; not NPU stall",
        "host_activation_to_enqueue_block_ms": context.ssd_depth_host_enqueue_wait_ms,
        "host_enqueue_wait_definition": "sum per issued command of enqueue minus layer I/O activation; includes original client issue pacing",
        "completed_host_activation_to_enqueue_block_ms": context.ssd_depth_completed_host_wait_ms,
        "completed_enqueue_to_hbm_block_ms": context.ssd_depth_completed_enqueued_latency_ms,
        "completed_activation_to_hbm_block_ms": context.ssd_depth_completed_activated_latency_ms,
        "activated_blocks": context.ssd_depth_activated_blocks,
        "issued_blocks": context.ssd_depth_issued_blocks,
        "ssd_completed_blocks": context.ssd_depth_completed_blocks,
        "hbm_completed_blocks": context.completed_blocks,
        "host_deferred_blocks_at_stop": host_pending,
        "ssd_outstanding_blocks_at_stop": outstanding,
        "link_outstanding_blocks_at_stop": link_pending,
        "input_not_yet_activated_blocks": context.expected_blocks-context.ssd_depth_activated_blocks,
    }
    return summary
...
def simulate_continuous_batch(
    ...
    *,
    ...
    An optional ``ssd_queue_depth_per_npu`` caps queued plus active commands
    independently per execution NPU and SSD. The host retains unissued work;
    SSD completion releases slots before link transfer. It does not cap
    bandwidth or change layer I/O activation timestamps.
    """
    ...
    if ssd_queue_depth_per_npu is not None:
        if (isinstance(ssd_queue_depth_per_npu, bool)
                or not isinstance(ssd_queue_depth_per_npu, (int, np.integer))
                or ssd_queue_depth_per_npu <= 0):
            raise ValueError("SSD queue depth per NPU must be a positive integer or None")
        ssd_queue_depth_per_npu = int(ssd_queue_depth_per_npu)
    ...
    context = _Context(
        ...
        ssd_queue_depth_per_npu=ssd_queue_depth_per_npu,
    )
    if disk_qos_configs:
    ...
    if steady_state is not None:
        ...
        summary = _build_steady_state_summary(context, current_time_ms, events_processed)
    else:
        summary = _build_summary(context, requests, current_time_ms, events_processed)
    return _add_ssd_queue_depth_summary(context, summary, current_time_ms)
