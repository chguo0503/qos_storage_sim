#!/usr/bin/env python3
"""Mixed L/S on every NPU, distinct profiles, native FIFO/once and ring placement.

C is measured-data bilinear interpolation (length below 32K explicitly
extrapolates). No arrival shifts, compute scaling, padding, or core changes.
"""
...
def profile(table, prefix, nql):
    ...
    return dict(total_tokens=tokens,total_length_k=seq,nql=nql,ssd_prefix_tokens=prefix,
        ...
        profile_construction=dict(method='direct_data_row' if direct else
            ...
            kv_formula='128-token aligned hit prefix * 1408 / 2**30'))


def pool_for(table, k, nql, jitter):
    prefix = int((k*1024-nql)//128)*128
    if jitter == 'raw':
        # Raw measured rows only; insufficient distinct rows cause a hard error.
        candidates=[]
        for x,y in table:
            if (k >= 96 and x >= 96) or (k < 96 and x < 96):
                candidates.append(profile(table,int(x*1024-y),y))
        return sorted(candidates,key=lambda p:(abs(p['total_length_k']-k)/max(k,1)+abs(p['nql']-nql)/nql,p['nql']))
    if jitter == 'narrow':
        # Fixed bytes, nearest available integer NQLs; build() trims this pool
        # to only modestly more profiles than the required no-replacement count.
        lower=max(64,nql-511) if k>=96 else nql
        candidates=[profile(table,prefix,y) for y in range(lower,min(4096,nql+511)+1)
                    if 20*1024 <= prefix+y <= 200*1024]
    else:
        low=max(20,k-max(2.,k*.12)); high=min(200,k+max(2.,k*.12))
        prefix_min=math.ceil((low*1024-nql)/128)*128
        prefix_max=math.floor((high*1024-nql)/128)*128
        ys=range(max(64,nql-max(16,nql//8)),min(4096,nql+max(16,nql//8))+1,8)
        candidates=[profile(table,x,y) for x in range(prefix_min,prefix_max+1,128) for y in ys
                    if 20*1024 <= x+y <= 200*1024]
    return sorted(candidates,key=lambda p:(abs(p['nql']-nql),abs(p['total_length_k']-k),p['nql']))
...
def build(args):
    ...
    for n in range(a.num_npu):
        ...
        for g,role in enumerate(role_deck):
            ...
            rid=n*1000000+g;blocks=p['ssd_prefix_tokens']//128
            layer=tuple((sim.block_ring_hash_disk_id(rid,j,a.ssu),IO) for j in range(blocks))
            ...
            vector=[disk_counts[s]*IO/(p['compute_us']/1e6) for s in range(a.ssu)]
    ...
    metadata=dict(experiment='fifo_mixed_unique_20260914',case=case,profiles=specs,
        num_npu=a.num_npu,num_ssu=a.ssu,n_layers=LAYERS,seed=a.seed,horizon_ms=a.horizon_ms,
        order=a.order,phase=a.phase,lane_checks=lane_checks,equal_176kib_blocks=True,
        ...
        'Derived compute profiles are interpolated from data; lengths below 32K extrapolate. No compute scaling or padding.')
...
def parser():
    ...
    p.add_argument('--long-k',type=float,default=200.);p.add_argument('--long-nql',type=int,default=2048)
    p.add_argument('--short-k',type=float,default=32.);p.add_argument('--short-nql',type=int,default=1280)
...
def main():
    ...
    observer=ReceiptObserver(requests,a.num_npu,a.ssu,a.window,policy=a.policy)
