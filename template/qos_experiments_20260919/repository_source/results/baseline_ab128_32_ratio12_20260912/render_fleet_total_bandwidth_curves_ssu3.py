#!/usr/bin/env python3
"""New two-line fleet total curves; never rewrite prior figures."""
...
OUT = HERE/'figures/ssu3/fleet_total_bandwidth_curves'


def combined(pieces,edges):
    changes=np.zeros(len(edges))
    for a,z,value in pieces:
        changes[np.searchsorted(edges,a)]+=value
        changes[np.searchsorted(edges,z)]-=value
...
def build(order,data):
    ...
    source.base.close(avg_B,reference['per_card_mean_demand_GiB_s']*32)
    source.base.close(avg_b,reference['per_card_mean_supply_GiB_s']*32)
    ...
    source.base.close(avg_b*2,physical_GiB)
    fig=plt.figure(figsize=(15,6.7),dpi=160,facecolor='white')
    fig.text(.065,.945,f'Baseline {order.title()}：32 张 NPU 的总带宽需求与供给',
             fontsize=23,color=source.INK)
    fig.text(.065,.894,'32 NPU / 3 SSU × 40 GiB/s · seed 7 · warm [2,4) 秒 · 纵轴是 32 卡总量',
             fontsize=13,color=source.MUTED)
    fig.text(.065,.850,f'整个窗口的时间平均：需求 {avg_B:.3f} GiB/s；实际供给 {avg_b:.3f} GiB/s',
             fontsize=13,color=source.INK)
    ax=fig.add_axes([.085,.225,.87,.555])
    ax.stairs(B,edges/1000,baseline=None,color=source.PURPLE,lw=2.3,ls='--',label='总需求：32 卡当前 B_i 相加')
    ax.stairs(b,edges/1000,baseline=None,color=source.BLUE,lw=2.0,label='总供给：各卡按层周期平均后，再对 32 卡相加')
    ax.set(xlim=(2,4),ylim=(0,1000),xticks=np.arange(2,4.01,.25),yticks=[0,200,400,600,800,1000],
           xlabel='仿真时间（秒）',ylabel='总带宽（GiB/s）')
    ax.spines[['top','right']].set_visible(False)
    ax.grid(alpha=.15)
    ax.legend(loc='lower left',bbox_to_anchor=(0,1.015),ncol=2,
              frameon=False,fontsize=10.5,borderaxespad=0)
    fig.text(.065,.115,'供给先按每张卡各自的层周期取平均（包含等待），再对 32 卡相加；不是原始瞬时传输速率。',
    ...
    fig.text(.065,.032,'总需求是 32 卡 V/C 参考值之和；两条线之比不能作为 NPU 利用率。',fontsize=11.5,color=source.MUTED)
    ...
    png=OUT/f'{order}_total_demand_supply.png'
    fig.savefig(png,dpi=160);plt.close(fig)
    with (OUT/f'{order}_curves.csv').open('w',newline='') as f:
        w=csv.writer(f);w.writerow(['start_ms','end_ms','total_demand_GiB_s','total_supply_GiB_s'])
        w.writerows(zip(edges[:-1],edges[1:],B,b))
    return dict(order=order,image=str(png.relative_to(ROOT)),sha256=source.base.sha(png),
                pixels=[width,height],time_mean_total_demand_GiB_s=avg_B,time_mean_total_supply_GiB_s=avg_b,
...
def main():
    ...
    (OUT/'README.md').write_text('''# 32 卡总需求与总供给：两条曲线

独立新增的PNG，Random、Ordered分开。32 NPU、3 SSU × 40 GiB/s、seed 7、warm [2,4) 秒。

- [Random PNG](random_total_demand_supply.png)
- [Ordered PNG](ordered_total_demand_supply.png)

横轴为仿真时间，纵轴为32张卡的总带宽（GiB/s）。每张图只有两条曲线：

1. 需求：同一时刻32张卡各自当前请求的B_i=V/C相加，不除以32。
2. 供给：先把每张卡实际收到的数据量摊到自己的层周期（当前层开始计算到下一层开始计算，包含等待），再在同一时刻对32张卡相加。
...
''')
    print(json.dumps([{k:r[k] for k in ('order','image','pixels','time_mean_total_demand_GiB_s','time_mean_total_supply_GiB_s')} for r in results],ensure_ascii=False),flush=True)
