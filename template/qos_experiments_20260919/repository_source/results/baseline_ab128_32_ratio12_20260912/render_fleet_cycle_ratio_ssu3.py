...
OUT=HERE/'figures/ssu3/fleet_cycle_ratio'
...
def analyse(order):
    ...
    assert man['metadata']['num_npu']==32 and man['metadata']['num_ssu']==3
    ...
    assert np.all(rows[:,4]==(rows[:,3]+rows[:,1])%3)
    ...
    return dict(order=order,seed=7,num_npu=32,num_ssu=3,window_ms=[LEFT,RIGHT],denominator_card_ms=DEN,
...
def draw(data):
    ...
    fig.text(.055,.942,f'Baseline {data["order"].title()} · 32 NPU / 3 SSU × 40 GiB/s · seed 7 · 同一个 warm 窗口 [2,4) 秒',fontsize=14,color=MUTED)
...
def main():
    ...
    by_order={d['order']:d for d in results}
    oa,ob,og=[by_order['ordered']['groups'][k] for k in ('A','B','gray')]
    example=next(c for c in by_order['ordered']['cycles'] if c['npu']==15 and c['group']=='A' and c['compute_layer']==2)
    md=['# 32 张 NPU：按层周期的 b_i/B_i 如何汇总成整机平均利用率','',
        '使用周六32 NPU、3 SSU × 40 GiB/s、长验证 seed 7 的既有日志；两图均统计同一个 warm [2,4) 秒。没有新增仿真，只生成 PNG。','',
    ...
    md+=['',f'Random真实整机U为{by_order["random"]["U_percent"]:.6f}%，Ordered为{by_order["ordered"]["U_percent"]:.6f}%。这里都是seed7，不能写成三种子均值。','',
        f'Ordered的完整A内部周期平均r约{oa["weighted_r"]:.4f}，占{oa["share_percent"]:.4f}%的窗口卡时间；这些周期贡献{oa["U_contribution_pp"]:.4f}个百分点计算利用率，等待损失{oa["loss_pp"]:.4f}个百分点。这是低r周期占用大量时间后怎样拉低整窗U的直接记账证据。B完整内部周期平均r为{ob["weighted_r"]:.4f}。','',
        '把Ordered表格直接代入，可以看到比值和最终U如何对上（下式有四舍五入）：','',
        '```text',f'A贡献：{oa["share_percent"]:.4f}% × {oa["weighted_r"]:.6f} = {oa["U_contribution_pp"]:.4f}个百分点',f'B贡献：{ob["share_percent"]:.4f}% × {ob["weighted_r"]:.6f} = {ob["U_contribution_pp"]:.4f}个百分点',
        f'灰色贡献：按日志统计的真实计算时间 / 64卡秒 = {og["U_contribution_pp"]:.4f}个百分点',
        f'整机U = {oa["U_contribution_pp"]:.4f}% + {ob["U_contribution_pp"]:.4f}% + {og["U_contribution_pp"]:.4f}% = {by_order["ordered"]["U_percent"]:.4f}%',
        f'A的等待损失 = {oa["share_percent"]:.4f}% × (1 − {oa["weighted_r"]:.6f}) = {oa["loss_pp"]:.4f}个百分点','```','',
        f'例如本次3盘NPU15的一个Ordered A内部周期：计算{example["C_ms"]:.3f}ms，等待{example["D_ms"]-example["C_ms"]:.3f}ms，共{example["D_ms"]:.3f}ms。实际接收{example["V_GiB"]*1024:.3f}MiB，摊到整轮平均为{example["mean_b_GiB_s"]:.6f}GiB/s；平均b/B={example["r"]:.6f}，也等于C/D。意思是这一轮约{100*example["r"]:.2f}%的时间在计算，其余在等。局部例子与整窗A平均可以不同。','',
        ...
        '[Random全部逐周期数值CSV](cycles_random.csv) · [Ordered全部逐周期数值CSV](cycles_ordered.csv) · [完整来源和校验](checks.json) · [生成代码](../../../render_fleet_cycle_ratio_ssu3.py)','']
