#!/usr/bin/env python3
"""Read-only four-NPU audit with deliberate repetition of exact raw-data A/B profiles.

Derived from audit_boundary_underload; all bandwidth, placement and stall gates retained.
...
DEFAULT = ROOT / 'results/boundary_exempt_underload_20260914'
CAP, LINK, EPS = 40., 50., 1e-8
KINDS = ('same_request', 'same_role_cross_request', 'cross_role_boundary')
...
def build(man, result, meta):
    ...
    for rid,q in qs.items():
        load = q['load']
        raw=table[(load['total_tokens']/1024,load['nql'])]
        assert not load['constructed_profile']
        assert math.isclose(raw[1],load['per_layer_us'],abs_tol=1e-8)
        assert math.isclose(raw[3],load['per_layer_kv_gb'],abs_tol=1e-12)
        ...
        assert load['total_tokens']==load['ssd_prefix_tokens']+load['nql']
        assert 32*1024<=load['total_tokens']<=200*1024 and 64<=load['nql']<=4096
        assert not load['profile_construction'].get('extrapolated',False)
        assert load['profile_construction'].get('compute_scale',1)==1
        assert math.isclose(load['per_layer_us'],load['original_compute_us'],abs_tol=1e-8)
    ...
    for n in range(N):
        ...
        if meta.get("all_length_nql_unique_within_each_npu",True):
            ...
            assert meta.get("exact_profile_repetition_intentional")
            assert meta.get("profiles_exactly_from_data") and len(set(combos))==2
    ...
    for n in range(N):
        ...
        for previous,following in zip(flat,flat[1:]):
            ...
            kind=('same_request' if old==rid else 'same_role_cross_request' if oldrole==role else 'cross_role_boundary')
...
def audit(case,out=DEFAULT,near_fraction=.9,max_near_percent=5.):
    ...
    boundary=kinds['cross_role_boundary']; continuing=kinds['same_request']+kinds['same_role_cross_request']
    ...
    group_profiles={gid:dict(role=d['qs'][rids[0]]['load']['role'],request_count=len(rids),
        ...
        max_own_npu_gib_s=max(math.fsum(d['ownrates'][r]) for r in rids)) for gid,rids in d['groups'].items()}
    result_audit=dict(schema_version=1,case=str(case),name=metrics['name'],policy=metrics['policy'],seed=meta['seed'],
        ...
        definitions=dict(current='Each admitted current request own per-SSU V/C over admission-to-completion, including stalled spans; never exempted.',
            continuing='Vnext/Ccurrent for same-request internal layers and same-role next-request L0, over actual current compute window.',
            boundary='Only cross-role S-to-L or L-to-S next-request L0 Vnext/Ccurrent contributions. S/L are input read-length role labels, not native QoS SS/LL classes.',
            exception='Total reference may exceed capacity only while cross-role boundary contributions are present; continuing contribution alone must stay within capacity. No time interval is removed from current-demand check.',
