#!/usr/bin/env python3
"""Native FIFO/Once experiments with a frozen, shuffled multi-profile input.

Only workload construction is new. The existing exact-tail adapter retains
the original per-IO SSD/NPU model and original periodic 5-ms Once policy.
"""
...
def build(spec, seed=7, horizon_ms=4500.):
    ...
    for g in groups:
        assert 32<=g['total_k']<=200 and g['role'] in ('L','S')
    ...
    meta=dict(experiment='random_multitype_search_20260914',case=dict(name=spec['name'],num_npu=N,ssu=S,
        ...
        all_compute_profiles_within_measured_grid=True,variable_tail_io_supported=True,
        ...
        'Interpolation stays inside the measured grid; no compute scaling or length extrapolation.')
...
def main():
    ...
    result.update(probe_policy=a.policy,demand_audit=audit,whole_run_demand_audit=whole,mixed_window_by_npu=lanes,exploration_case=meta['case'])
    ...
    row=dict(name=spec['name'],num_npu=8,num_ssu=meta['num_ssu'],seed=a.seed,policy=a.policy,window_ms=a.window,
