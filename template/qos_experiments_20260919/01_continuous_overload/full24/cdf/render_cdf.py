"""Render the original full-load CDF with two requested strategy labels."""
...
for policy, (label, color, style) in policies.items():
    data = sorted((r for r in rows if r['scenario'] == 'full' and r['policy'] == policy),
    ...
    assert y[-1] == 1 and x[0] > .9 and x[-1] < 17
    at_slo = float(y[np.flatnonzero(x == 1.5)[0]])
    expected = .4029543087150067 if policy == 'baseline' else .7613026819923371
    assert abs(at_slo - expected) < 1e-12
    xx, yy = np.r_[.9, x, 17.], np.r_[0., y, 1.]
...
ax.set(xlim=(.9, 17), ylim=(0, 1.035), ylabel='累计请求比例（各种子 CDF 的均值）',
...
fig.text(.095, .950, '持续过载：两种策略的归一化耗时 CDF',
...
stem = HERE / 'full_random_ttft_ratio_cdf_two_strategies'
...
(HERE / 'checks.json').write_text(json.dumps({'no_new_simulation': True,
    ...
    'cdf_values_preserved': True, 'axis_range': [.9, 17]}, ensure_ascii=False, indent=2))
